...
// LexIA · CRM — Honorário (fee-lançamento) detail modal. Ported from
// page-contratos.jsx, wired to the real backend: fetches HonorarioDetail
// (honorário + série + parcelas from the cash ledger) and runs pagar/desmarcar
// against the finance routes. Opened from within a Contrato (CrmContratoModal)
// or from the cliente's Financeiro tab — a honorário is a RECEBÍVEL, not a
// contrato (see CrmContratoModal for the real, cliente-facing contract entity).
...
export function CrmHonorarioModal({ honorarioId, dataset, onClose, onRefresh, nav }: Props) {
  ...
  const load = useCallback(async () => {
    try {
      const d = await fetchHonorarioDetail(honorarioId)
      setH(d)
  ...
  return (
    <FxModal
      title={loading ? "Carregando…" : h ? h.descricao : "Honorário"}
    ...
    >
      {loading || !h ? (
        <div style={{ padding: "40px 0", textAlign: "center", color: "var(--text-muted)", fontSize: 13 }}>
          {loading ? "Carregando honorário…" : "Honorário não encontrado."}
        </div>
      ) : (
