...
import { CmCardTitle, CmEmpty, CmFrame, CmNum } from "../cm-kit"
import { cmCompact, cmFunnel, cmPct, type CmRef, type CmScope, type FunnelResult, type FunnelStage, type Periodo } from "../cm-meta"
import type { CmDataset, LeadEtapa } from "@/lib/comercial/types"

function FunnelChart({ stages, onStage }: { stages: FunnelStage[]; onStage: (k: LeadEtapa) => void }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
      {stages.map((s, i) => {
        ...
        return (
          <div key={s.key}>
            <button onClick={() => onStage(s.key)} className="cm-funnel-row" style={{ display: "flex", alignItems: "center", gap: 14, width: "100%", textAlign: "left", padding: "12px 14px", border: "none", background: "transparent", borderRadius: 10, cursor: "pointer" }}>
...
export function CmFunil({ dataset, ref0, period, scope, onStage, onLead }: { dataset: CmDataset; ref0: CmRef; period: Periodo; scope: CmScope; onStage: (k: LeadEtapa) => void; onLead: () => void }) {
  const f: FunnelResult = useMemo(() => cmFunnel(dataset.leads, ref0, period), [dataset.leads, ref0, period])
    ...
    <div style={{ flex: 1, minHeight: 0, overflow: "auto" }}>
      <CmFrame>
        <div style={{ marginBottom: 20 }}>
          <h1 style={{ margin: 0, fontSize: 25, fontWeight: 500, letterSpacing: "-0.025em", color: "var(--text)" }}>Funil de vendas · {scope.title}</h1>
          <p style={{ margin: "4px 0 0", fontSize: 14, color: "var(--text-muted)" }}>{f.total} leads no período · {cmPct(f.stages[4].conv, 1)} de conversão ponta a ponta · clique numa etapa para ver os leads</p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 20, alignItems: "start" }}>
          <div className="card" style={{ padding: "16px 14px" }}>
            ...
            <FunnelChart stages={f.stages} onStage={onStage} />
          </div>
            ...
            <div className="card" style={{ padding: "18px 22px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                <button className="cm-rowlink" onClick={() => onStage("ganho")} style={{ textAlign: "left", border: "1px solid var(--border)", background: "var(--bg-soft)", borderRadius: 10, padding: "11px 13px", cursor: "pointer" }}>
                  ...
                  <div style={{ fontSize: 20, fontWeight: 500, color: "var(--text)", marginTop: 4 }}>{f.ganho}</div>
                ...
                <button className="cm-rowlink" onClick={() => onStage("perdido")} style={{ textAlign: "left", border: "1px solid var(--border)", background: "var(--bg-soft)", borderRadius: 10, padding: "11px 13px", cursor: "pointer" }}>
                  ...
                  <div style={{ fontSize: 20, fontWeight: 500, color: "var(--text)", marginTop: 4 }}>{f.perdido}</div>
                    ...
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 11 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ height: 5, borderRadius: 4, background: "var(--bg-sunken)", marginTop: 4, overflow: "hidden" }}>
                          <div style={{ width: `${g.conv}%`, height: "100%", background: g.conv < 50 ? "#C0492F" : "var(--accent-strong)", borderRadius: 4, opacity: 0.9 }} />
                        </div>
                      </div>
