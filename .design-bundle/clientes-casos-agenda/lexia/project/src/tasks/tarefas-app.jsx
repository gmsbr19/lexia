  ...
  return (
    <div className="card" style={{ padding: hasTokens ? '11px 14px 9px' : '11px 14px', marginBottom: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
        ...
        <button className="btn btn-primary" onClick={submit} disabled={!parsed || !parsed.title} style={{ height: 30, fontSize: 12, opacity: parsed && parsed.title ? 1 : 0.5 }}>Adicionar</button>
      </div>
      {hasTokens && (
        <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginTop: 9, paddingTop: 9, borderTop: '1px solid var(--border)', flexWrap: 'wrap' }}>
          ...
          {parsed.prio && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontSize: 11, fontWeight: 500, color: PRIO[parsed.prio].color, background: 'var(--bg-sunken)', padding: '2px 8px', borderRadius: 999 }}><Icon name="flag" size={10} strokeWidth={2.2} />{PRIO[parsed.prio].label}</span>}
        </div>
      )}
    </div>
  );
...
const PlanoBanner = ({ onGenerate }) => (
  <div style={{
    ...
    <div style={{ position: 'relative', flex: 1, minWidth: 0 }}>
      <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text)', letterSpacing: '-0.01em' }}>Plano da semana · LexIA</div>
    ...
    <button className="btn btn-gold" onClick={onGenerate} style={{ height: 34, fontSize: 12, flexShrink: 0 }}>
...
const FilterBtn = ({ icon, label, active, children }) => (
  <span style={{
    ...
    cursor: 'pointer', fontSize: 12, fontWeight: 500, color: active ? 'var(--accent)' : 'var(--text-muted)', whiteSpace: 'nowrap',
    ...
    <AppShell
      ...
      actions={
        <>
          ...
          <button className="btn btn-primary" onClick={newBlank} style={{ height: 32, fontSize: 12 }}><Icon name="plus" size={13} strokeWidth={2.2} />Nova tarefa</button>
        </>
      }
    >
      <PageFrame>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18, flexWrap: 'wrap' }}>
          ...
          <button onClick={() => setOnlyMine(v => !v)} style={{
            ...
            fontSize: 12, fontWeight: 500, color: onlyMine ? 'var(--accent)' : 'var(--text-muted)', whiteSpace: 'nowrap',
          }}>
            <Icon name="inbox" size={14} strokeWidth={1.85} />Atribuídas a mim
            <span style={{ fontSize: 11, fontWeight: 500, background: onlyMine ? 'var(--accent)' : 'var(--bg-sunken)', color: onlyMine ? '#fff' : 'var(--text-subtle)', padding: '1px 6px', borderRadius: 999 }}>{mineCount}</span>
          </button>
        </div>
