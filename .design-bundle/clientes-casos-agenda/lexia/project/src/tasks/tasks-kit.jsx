  ...
  return (
    <span
      ...
      style={{
        ...
        padding: '2px 8px 2px 6px', borderRadius: 6, cursor: 'pointer', maxWidth: 160,
      }}
    >
...
const DataChip = ({ data, hora, faded }) => {
  ...
  return (
    <span style={{
      ...
      fontSize: 12, fontWeight: 500, color: faded ? 'var(--text-subtle)' : (today ? 'var(--accent)' : 'var(--text-muted)'),
    }}>
...
const PrazoChip = ({ prazo, done, compact = false }) => {
  ...
  return (
    <span style={{
      ...
      padding: t.bg === 'transparent' ? '0' : '2px 8px', borderRadius: 6,
    }}>
...
const SectionHeader = ({ icon, label, count, tone, right }) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 2px 9px', borderBottom: '1px solid var(--border)', marginBottom: 4 }}>
      ...
      <span style={{ fontSize: 13, fontWeight: 500, color, letterSpacing: '-0.01em' }}>{label}</span>
