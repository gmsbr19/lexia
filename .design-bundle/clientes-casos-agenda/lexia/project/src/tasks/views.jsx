  ...
  <div onClick={() => onOpen(task.id)} className="task-row" style={{
    ...
    <div style={{ flex: 1, minWidth: 0 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
        <span style={{ fontSize: 14, fontWeight: 500, color: 'var(--text)', letterSpacing: '-0.01em', textDecoration: task.done ? 'line-through' : 'none', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{task.title}</span>
        {task.ai && <IaBadge />}
      ...
      {overdue.length > 0 && (
        <RowGroup>
          <SectionHeader icon="flame" label="Atrasadas" count={overdue.length} tone="vencido"
            right={<span style={{ fontSize: 12, color: 'var(--fin-neg, #C0492F)', fontWeight: 500, cursor: 'pointer' }}>Reagendar todas</span>} />
  ...
  return (
    <div className="card" style={{ padding: 16 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14 }}>
        <span style={{ fontSize: 15, fontWeight: 500, color: 'var(--text)', letterSpacing: '-0.01em' }}>{['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'][ref.m]} {ref.y}</span>
        ...
        <div style={{ flex: 1 }} />
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--text-subtle)' }}><span style={{ width: 8, height: 8, borderRadius: 2, background: 'var(--accent)' }} />agendada</span>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--text-subtle)' }}><Icon name="flag" size={11} style={{ color: 'var(--fin-neg, #C0492F)' }} />prazo</span>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 1, background: 'var(--border)', border: '1px solid var(--border)', borderRadius: 10, overflow: 'hidden' }}>
        {['dom', 'seg', 'ter', 'qua', 'qui', 'sex', 'sáb'].map(w => (
          <div key={w} style={{ background: 'var(--bg-soft)', padding: '7px 8px', fontSize: 11, fontWeight: 500, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{w}</div>
        ))}
        {cells.map((d, i) => {
            ...
            <div key={i} style={{ background: 'var(--surface)', minHeight: 92, padding: 6, display: d ? 'flex' : 'block', flexDirection: 'column', gap: 3 }}>
              {d && (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 2 }}>
                  <span style={{
                    fontSize: 12, fontWeight: isToday ? 700 : 500, fontFeatureSettings: '"tnum"',
                    color: isToday ? '#fff' : 'var(--text-muted)',
                    background: isToday ? 'var(--accent)' : 'transparent', width: 20, height: 20, borderRadius: '50%',
                    display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                  }}>{d}</span>
                  {deadlines > 0 && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 2, fontSize: 10, fontWeight: 500, color: 'var(--fin-neg, #C0492F)' }}><Icon name="flag" size={10} strokeWidth={2.2} />{deadlines}</span>}
                </div>
              )}
              {scheduled.slice(0, 3).map(t => (
                <div key={t.id} onClick={() => cb.onOpen(t.id)} title={t.title} style={{
                  fontSize: 11, fontWeight: 500, color: 'var(--text)', background: 'var(--bg-sunken)',
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '236px 1fr', gap: 16, alignItems: 'start' }}>
      ...
      <div className="card" style={{ padding: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)' }}>A agendar hoje</span>
          ...
          {pool.map(t => (
            <div key={t.id} draggable
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                ...
                <span style={{ flex: 1, fontSize: 12, fontWeight: 500, color: 'var(--text)', lineHeight: 1.3, minWidth: 0 }}>{t.title}</span>
              </div>
          ...
          {placed.map(t => {
            ...
            return (
              <div key={t.id} draggable
                ...
                <span style={{ flex: 1, fontSize: 12, fontWeight: 500, color: 'var(--text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', textDecoration: t.done ? 'line-through' : 'none' }}>{t.title}</span>
