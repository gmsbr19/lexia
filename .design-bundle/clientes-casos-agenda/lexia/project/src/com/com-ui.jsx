...
const CmKpi = ({ label, value, delta, deltaInvert, sub, icon, accent, tone, big }) => (
  <div className="card" style={{ padding: '15px 17px', display: 'flex', flexDirection: 'column', gap: 8, minHeight: 108 }}>
    ...
    <span style={{
      fontSize: big ? 27 : 24, fontWeight: 500, letterSpacing: '-0.025em', fontFeatureSettings: '"tnum"', lineHeight: 1.05,
...
const CmStagePill = ({ etapa }) => {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11, fontWeight: 500, padding: '3px 9px 3px 8px', borderRadius: 6, color: s.color, background: s.color + '22', whiteSpace: 'nowrap' }}>
...
const CmStatusChip = ({ status }) => {
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11, fontWeight: 500, color: t.c, whiteSpace: 'nowrap' }}>
      ...
      {options.map((o) => {
        ...
        return (
          <button key={val} onClick={() => onChange(val)} style={{
            ...
            fontSize: 12, fontWeight: on ? 600 : 500, fontFamily: 'var(--font-sans)', letterSpacing: '-0.01em',
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 40px', borderBottom: '1px solid var(--border)', background: 'var(--bg-soft)', flexShrink: 0, flexWrap: 'wrap', rowGap: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
        ...
        <div style={{ minWidth: 150, textAlign: 'center' }}>
          <div style={{ fontSize: 15, fontWeight: 500, color: 'var(--text)', letterSpacing: '-0.02em', lineHeight: 1.15 }}>{scopeLabel.title}</div>
    ...
    {tabs.map((t) => {
      ...
      return (
        <div key={t.id} onClick={() => onChange(t.id)} style={{
          ...
          fontSize: 14, fontWeight: on ? 600 : 500, color: on ? 'var(--text)' : 'var(--text-muted)',
...
const CmInput = (props) => <input {...props} className="input" style={{ height: 38, fontSize: 14, ...(props.style || {}) }} />;
const CmSelect = ({ options, value, onChange, placeholder, ...rest }) => (
  <div style={{ position: 'relative' }}>
    <select value={value} onChange={onChange} {...rest} className="input" style={{ height: 38, fontSize: 14, appearance: 'none', WebkitAppearance: 'none', paddingRight: 34, cursor: 'pointer' }}>
    ...
    <div onMouseDown={onClose} style={{ position: 'absolute', inset: 0, zIndex: 60, display: 'flex', alignItems: 'stretch', justifyContent: 'flex-end', background: 'rgba(2,13,37,0.42)', backdropFilter: 'blur(3px)' }}>
      <div onMouseDown={(e) => e.stopPropagation()} className="cm-slidein" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12, padding: '20px 24px 16px', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
          <div>
            <div style={{ fontSize: 17, fontWeight: 500, color: 'var(--text)', letterSpacing: '-0.02em' }}>{title}</div>
...
const CmTh = ({ children, align = 'left', w }) => (
  <th style={{ textAlign: align, padding: '11px 14px', fontSize: 11, fontWeight: 500, color: 'var(--text-subtle)', letterSpacing: '0.07em', textTransform: 'uppercase', whiteSpace: 'nowrap', width: w }}>{children}</th>
);
