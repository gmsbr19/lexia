  ...
  return (
    <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
      ...
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <span style={{ fontSize: 22, fontWeight: 500, color: 'var(--text)', letterSpacing: '-0.03em', fontFeatureSettings: '"tnum"' }}>{Math.round(pct * 100)}%</span>
      </div>
    </div>
  );
};
  ...
  return (
    <div style={{
      ...
      <div style={{ position: 'relative', flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, fontWeight: 500, color: 'var(--accent)', letterSpacing: '0.04em', textTransform: 'uppercase', marginBottom: 10 }}>
          <Icon name="sparkles" size={12} strokeWidth={2} />
          Plano gerado pela LexIA
        </div>
        <div style={{ fontSize: 30, fontWeight: 500, color: 'var(--text)', letterSpacing: '-0.03em', lineHeight: 1.1 }}>
          Recupere até <span style={{ color: 'var(--fin-pos, #2E9E5B)' }}>{planMoney(PLAN_TOTAL_VALUE)}</span> em receita
        </div>
        <p style={{ margin: '8px 0 0', fontSize: 14, color: 'var(--text-muted)', lineHeight: 1.55, maxWidth: 520 }}>
      ...
      <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 18, paddingLeft: 28, borderLeft: '1px solid var(--border)', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
          <div>
            <div style={{ fontSize: 12, color: 'var(--text-subtle)', marginBottom: 1 }}>Ações concluídas</div>
            <div style={{ fontSize: 17, fontWeight: 500, color: 'var(--text)', fontFeatureSettings: '"tnum"' }}>{doneCount} <span style={{ color: 'var(--text-subtle)', fontWeight: 500 }}>/ {PLAN_TOTAL_COUNT}</span></div>
          </div>
          <div>
            <div style={{ fontSize: 12, color: 'var(--text-subtle)', marginBottom: 1 }}>Receita recuperada</div>
            <div style={{ fontSize: 17, fontWeight: 500, color: recovered > 0 ? 'var(--fin-pos, #2E9E5B)' : 'var(--text)', fontFamily: 'var(--font-mono)', fontFeatureSettings: '"tnum"' }}>{planMoney(recovered)}</div>
          </div>
          {allDone && (
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11, fontWeight: 500, color: 'var(--fin-pos, #2E9E5B)' }}>
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {step.ai && (
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: 9.5, fontWeight: 500, color: 'var(--accent)', background: 'var(--accent-soft)', padding: '1px 6px', borderRadius: 999, flexShrink: 0 }}>
              <Icon name="sparkles" size={9} strokeWidth={2} />IA
            </span>
          )}
          <span style={{ fontSize: 10, fontWeight: 500, color: pr.fg, background: pr.bg, padding: '2px 7px', borderRadius: 6, flexShrink: 0, letterSpacing: '0.01em' }}>{step.priority}</span>
        </div>
        <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 3, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{step.ctx}</div>
      </div>

      {step.value > 0 && (
        <div style={{ textAlign: 'right', flexShrink: 0, marginRight: 4 }}>
          <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text)', fontFamily: 'var(--font-mono)', fontFeatureSettings: '"tnum"', letterSpacing: '-0.01em' }}>{planMoney(step.value)}</div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 6 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
            <span style={{ fontSize: 14.5, fontWeight: 500, color: 'var(--text)', letterSpacing: '-0.015em' }}>{group.title}</span>
            {groupValue > 0 && (
              <span style={{ fontSize: 11, fontWeight: 500, color: t.dot, background: t.soft, padding: '2px 9px', borderRadius: 6, fontFeatureSettings: '"tnum"' }}>{planMoney(groupValue)}</span>
            )}
          </div>
          <div style={{ fontSize: 12, color: 'var(--text-subtle)', marginTop: 2 }}>{group.desc}</div>
        </div>
        <span style={{ fontSize: 12, fontWeight: 500, color: done === total ? 'var(--fin-pos, #2E9E5B)' : 'var(--text-muted)', fontFeatureSettings: '"tnum"', flexShrink: 0 }}>{done}/{total}</span>
      </div>
      <div>
        {group.steps.map((s, i) => (
...
const PlanReasoning = () => (
  <div style={{
    ...
    <div style={{ position: 'relative' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, fontWeight: 500, color: 'var(--accent)', letterSpacing: '0.04em', textTransform: 'uppercase', marginBottom: 12 }}>
        <Icon name="sparkles" size={12} strokeWidth={2} />
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
        {[
          ...
          <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
            ...
            <p style={{ margin: 0, fontSize: 12, lineHeight: 1.5, color: 'var(--text-muted)' }}>{txt}</p>
          </div>
        ))}
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 14, paddingTop: 13, borderTop: '1px solid var(--border)' }}>
        ...
        <div style={{ flex: 1 }} />
        <span style={{ fontSize: 11, color: 'var(--text-subtle)' }}>há 2 h</span>
      </div>
    </div>
  </div>
);
...
const PlanTimeline = () => (
  <div className="card" style={{ padding: '16px 18px' }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
      ...
      <span style={{ fontSize: 14, fontWeight: 500, color: 'var(--text)', letterSpacing: '-0.01em' }}>Sequência sugerida</span>
    </div>
        ...
        return (
          <div key={i} style={{ display: 'flex', gap: 11 }}>
            ...
            <div style={{ paddingBottom: i < TIMELINE.length - 1 ? 2 : 0 }}>
              <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--text)', marginBottom: 4 }}>{blk.when}</div>
              <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 3 }}>
                {blk.items.map((it, j) => (
                  <li key={j} style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.4 }}>{it}</li>
                ))}
              </ul>
            </div>
          </div>
    ...
    >
      <PageFrame>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16, marginBottom: 20 }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 25, fontWeight: 500, letterSpacing: '-0.025em', color: 'var(--text)' }}>Plano de ação</h1>
