...
const finActions = (
  <>
    <button className="btn btn-secondary" style={{ height: 32, fontSize: 12 }}><Icon name="calendar" size={13} />Ano fiscal 2026</button>
    <button className="btn btn-primary" style={{ height: 32, fontSize: 12 }}><Icon name="upload" size={13} />Importar</button>
  </>
);
...
const MonthDetailRow = ({ d, last }) => {
  ...
  return (
    <tr style={{ borderTop: '1px solid var(--border)' }}>
      <Td><span style={{ fontWeight: 500, color: 'var(--text)' }}>{d.mes} 2026</span>{d.proj && <span style={{ marginLeft: 8 }}><StatusPill label="projeção" tone="alerta" dot={false} /></span>}</Td>
      <Td align="right"><MoneyValue value={d.recebido} size={12.5} /></Td>
      <Td align="right"><span style={{ color: 'var(--accent)', fontFamily: 'var(--font-mono)', fontSize: 12, fontWeight: 500, fontFeatureSettings: '"tnum"' }}>{fmtBRL(d.receber)}</span></Td>
      <Td align="right"><MoneyValue value={total} size={12.5} weight={500} /></Td>
...
const InadRow = ({ r, last }) => {
  ...
  return (
    <tr style={{ borderTop: '1px solid var(--border)' }}>
      ...
      <Td align="right">
        <button className="btn btn-secondary" style={{ height: 28, fontSize: 12, padding: '0 10px' }}><Icon name="phone" size={12} />Cobrar</button>
      </Td>
    </tr>
  );
};
...
const SubHead2 = ({ children }) => (
  <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)', letterSpacing: '-0.01em', marginBottom: 12 }}>{children}</div>
);
