...
const QuickAction = ({ icon, title, sub, badge }) => (
  <div className="card" style={{
    ...
    <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <div style={{ fontSize: 12, color: 'var(--text-subtle)', marginTop: 1 }}>{sub}</div>
    </div>
    {badge && <span style={{ fontSize: 11, fontWeight: 500, color: 'var(--accent)', background: 'var(--accent-soft)', padding: '2px 8px', borderRadius: 999 }}>{badge}</span>}
...
const HomeGreeting = ({ children }) => (
  <div style={{ marginBottom: 22 }}>
    <h1 style={{ margin: 0, fontSize: 25, fontWeight: 500, letterSpacing: '-0.025em', color: 'var(--text)' }}>Bom dia, Rafael</h1>
...
const SubHead = ({ children, action }) => (
  <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 13 }}>
    <h2 style={{ margin: 0, fontSize: 14, fontWeight: 500, letterSpacing: '-0.015em', color: 'var(--text)' }}>{children}</h2>
...
const homeShell = (children) => (
  <AppShell
    active="inicio"
    breadcrumb={['Início']}
    actions={<button className="btn btn-primary" style={{ height: 32, fontSize: 12 }}><Icon name="plus" size={13} />Novo documento</button>}
  >{children}</AppShell>
);
        ...
        {HOME_KPIS.map((k, i) => (
          <div key={i} className="card" style={{ padding: '13px 15px', display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 18, fontWeight: 500, color: 'var(--text)', letterSpacing: '-0.02em', fontFeatureSettings: '"tnum"' }}>{k.value}</div>
            </div>
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: 2, fontSize: 11, fontWeight: 500, color: k.deltaDir === 'down' ? 'var(--fin-neg,#C0492F)' : 'var(--fin-pos,#2E9E5B)' }}>
