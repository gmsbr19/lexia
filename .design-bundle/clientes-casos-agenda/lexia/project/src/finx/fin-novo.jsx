      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 15 }}>
            ...
            {[['in', 'A receber', 'arrowDownRight', '#2E9E5B'], ['out', 'A pagar', 'arrowUpRight', '#C0492F']].map(([d, t, ic, c]) => {
              ...
              return (
                <button key={d} onClick={() => setDirSafe(d)} style={{
                  ...
                  <span style={{ fontSize: 14, fontWeight: 500, color: on ? 'var(--text)' : 'var(--text-muted)' }}>{t}</span>
                </button>
              );
            })}
          </div>
        ...
        <div style={half}>
          ...
          <FxField label={labelParty}>
            <input className="input" list="fx-party-list" value={party} onChange={(e) => setParty(e.target.value)} placeholder="Selecione ou digite" style={{ height: 38, fontSize: 14 }} />
            ...
            {modo !== 'unica' && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 10, padding: '11px 14px', background: 'var(--bg-soft)', borderRadius: 'var(--r-sm)', border: '1px solid var(--border)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', border: '1px solid var(--border-strong)', borderRadius: 8, overflow: 'hidden' }}>
                  ...
                  <span style={{ width: 34, textAlign: 'center', fontSize: 15, fontWeight: 500, ...FX_NUM, color: 'var(--text)' }}>{vezes}</span>
                ...
                <div style={{ marginLeft: 'auto', textAlign: 'right' }}>
                  ...
                  <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text)', ...FX_NUM }}>= {fxMoney(preview.total)}</div>
                </div>
              </div>
            )}
          </FxField>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '0 14px', height: 46, borderRadius: 'var(--r-sm)', border: '1px solid var(--border)', background: 'var(--bg-soft)' }}>
          ...
          {pago
            ...
            : <span style={{ marginLeft: 'auto', fontSize: 12, color: 'var(--text-subtle)' }}>{!isEdit && modo !== 'unica' ? 'dê baixa em cada parcela depois' : ''}</span>}
        </div>
    ...
    >
      <div style={{ display: 'flex', gap: 20, padding: '4px 2px 16px', marginBottom: 4, borderBottom: '1px solid var(--border)' }}>
        {[['Total da série', total, 'var(--text)'], [dir === 'in' ? 'Recebido' : 'Pago', recebido, '#2E9E5B'], ['Em aberto', aberto, 'var(--accent)']].map(([l, v, c]) => (
          <div key={l}>
            <div style={{ fontSize: 11, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 500 }}>{l}</div>
            <div style={{ fontSize: 17, fontWeight: 500, color: c, ...FX_NUM, marginTop: 2 }}>{fxMoney(v)}</div>
          </div>
        ))}
      </div>
      <div style={{ fontSize: 12, color: 'var(--text-subtle)', margin: '14px 0 8px' }}>Edite qualquer parcela isoladamente — útil para dação, descontos ou renegociação.</div>
        ...
        {sorted.map((it, i) => {
          ...
          return (
            <div key={it.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 0', borderTop: i ? '1px solid var(--border)' : 'none' }}>
              <span style={{ width: 30, height: 30, borderRadius: 8, background: 'var(--bg-sunken)', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 500, ...FX_NUM, flexShrink: 0 }}>{i + 1}</span>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--text)' }}>{it.grupo || it.desc}</div>
