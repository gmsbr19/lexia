  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 12 }}>
        <div>
          <div style={{ fontSize: 12, color: 'var(--text-subtle)', fontWeight: 500 }}>Leandro</div>
          <div style={{ fontSize: 28, fontWeight: 500, letterSpacing: '-0.03em', color: 'var(--text)', fontVariantNumeric: 'tabular-nums' }}>{leandro}<span style={{ fontSize: 16, color: 'var(--text-subtle)' }}>%</span></div>
          <div style={{ fontSize: 12, color: 'var(--accent)', fontWeight: 500, fontVariantNumeric: 'tabular-nums' }}>{fxMoney(Math.round(total * leandro / 100))}</div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 12, color: 'var(--text-subtle)', fontWeight: 500 }}>Leonardo</div>
          <div style={{ fontSize: 28, fontWeight: 500, letterSpacing: '-0.03em', color: 'var(--text)', fontVariantNumeric: 'tabular-nums' }}>{leonardo}<span style={{ fontSize: 16, color: 'var(--text-subtle)' }}>%</span></div>
          <div style={{ fontSize: 12, color: 'var(--accent)', fontWeight: 500, fontVariantNumeric: 'tabular-nums' }}>{fxMoney(Math.round(total * leonardo / 100))}</div>
        </div>
      </div>
      <div ref={trackRef} onMouseDown={(e) => { setDrag(true); setFromClientX(e.clientX); }} style={{ position: 'relative', height: 38, cursor: 'pointer', userSelect: 'none', padding: '14px 0' }}>
        <div style={{ position: 'relative', height: 10, borderRadius: 999, overflow: 'hidden', background: 'var(--bg-sunken)' }}>
          <div style={{ position: 'absolute', inset: 0, width: `${leandro}%`, background: 'var(--brand-gold)' }}></div>
      ...
      <div onMouseDown={(e) => e.stopPropagation()} className="crm-pop-in card" style={{ width: 720, maxWidth: '100%', maxHeight: '92%', display: 'flex', flexDirection: 'column', boxShadow: 'var(--shadow-lg)', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '18px 22px', borderBottom: '1px solid var(--border)' }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 500, letterSpacing: '-0.02em', color: 'var(--text)' }}>{k.titulo}</div>
        ...
        <div style={{ overflowY: 'auto', padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 22 }}>
            ...
            <div className="card" style={{ overflow: 'hidden' }}>
              {fin.lancamentos.length === 0 ? <CrmEmpty icon="wallet" title="Sem lançamentos" /> : fin.lancamentos.map((l, i) => (
                <div key={l.id} className="crm-row" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', borderTop: i ? '1px solid var(--border)' : 'none' }}>
                  ...
                  <span style={{ flex: 1, fontSize: 12, color: 'var(--text)', minWidth: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{l.descricao}</span>
                  <span style={{ fontSize: 12, color: 'var(--text-subtle)', fontVariantNumeric: 'tabular-nums' }}>{fxDate(l.venc)}</span>
          ...
          {role !== 'staff' && (
            <div>
              ...
              <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 14 }}>Divisão dos honorários deste caso. Pontos de atração em 0, 50 e 100%.</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--text)' }}>Tarefas</div>
                <button className="btn btn-ghost" onClick={() => mut.openTarefa(k.clienteId)} style={{ height: 26, fontSize: 12, padding: '0 8px' }}><Icon name="plus" size={12} />Nova</button>
              </div>
              <div className="card" style={{ overflow: 'hidden' }}>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--text)' }}>Eventos</div>
                <button className="btn btn-ghost" onClick={() => mut.action('novo-evento')} style={{ height: 26, fontSize: 12, padding: '0 8px' }}><Icon name="plus" size={12} />Novo</button>
              </div>
              <div className="card" style={{ overflow: 'hidden' }}>
...
const CrmCasosPage = ({ store, role, onOpenCaso }) => {
  ...
  return (
    <FxFrame>
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: CRM_CASO_COLS, gap: 14, padding: '11px 18px', borderBottom: '1px solid var(--border)', background: 'var(--bg-soft)' }}>
          {['Caso', 'Tipo', 'Honorários', 'Rateio (sócios)'].map((h, i) => (
            <div key={h} style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.08em', textAlign: i >= 2 ? 'right' : 'left' }}>{h}</div>
          ))}
        </div>
          ...
          return (
            <CrmRow key={k.id} onClick={() => onOpenCaso(k.id)} style={{ display: 'grid', gridTemplateColumns: CRM_CASO_COLS, gap: 14, padding: '13px 18px', alignItems: 'center', borderTop: i ? '1px solid var(--border)' : 'none' }}>
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--text-subtle)', marginTop: 2 }}>{cli ? (cli.apelido || cli.nome) : ''} · resp. {crmFirst(k.responsavel)}</div>
              </div>
              <div><CrmCasoTipoPill tipo={k.tipo} /></div>
              <div style={{ textAlign: 'right', fontSize: 13, fontWeight: 500, fontVariantNumeric: 'tabular-nums', color: f.total ? 'var(--text)' : 'var(--text-subtle)' }}>{f.total ? fxMoney(f.total) : '—'}</div>
                ...
                {role === 'staff' ? <span style={{ fontSize: 12, color: 'var(--text-subtle)' }}>—</span> : (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <div style={{ width: 54, height: 7, borderRadius: 999, overflow: 'hidden', background: 'var(--brand-navy)', display: 'flex' }}>
                      <div style={{ width: `${k.rateio.leandro}%`, background: 'var(--brand-gold)' }}></div>
                    </div>
                    <span style={{ fontSize: 12, fontWeight: 500, color: 'var(--text-muted)', fontVariantNumeric: 'tabular-nums' }}>{k.rateio.leandro}/{k.rateio.leonardo}</span>
                  </div>
                )}
              </div>
            </CrmRow>
      ...
      <div style={{ fontSize: 12, color: 'var(--text-subtle)', textAlign: 'center', marginTop: 14 }}>{rows.length} de {store.casos.length} casos</div>
    </FxFrame>
  );
};
