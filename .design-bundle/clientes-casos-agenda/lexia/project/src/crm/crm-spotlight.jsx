      ...
      <div onMouseDown={(e) => e.stopPropagation()} className="crm-pop-in" style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '16px 18px', borderBottom: '1px solid var(--border)' }}>
          ...
          <input ref={inputRef} value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar clientes, casos, contratos, ações…"
            style={{ flex: 1, border: 'none', outline: 'none', background: 'transparent', fontFamily: 'var(--font-sans)', fontSize: 16, color: 'var(--text)', letterSpacing: '-0.01em' }} />
          <kbd style={{ fontSize: 11, color: 'var(--text-subtle)', background: 'var(--bg-sunken)', padding: '3px 7px', borderRadius: 5, fontFamily: 'var(--font-mono)' }}>esc</kbd>
        </div>

        <div ref={listRef} style={{ overflowY: 'auto', padding: '8px 8px 10px' }}>
          {flat.length === 0 && (
            <div style={{ padding: '36px 20px', textAlign: 'center' }}>
              <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text)' }}>Nenhum resultado para “{q}”</div>
              <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 5 }}>Tente outro termo ou crie algo novo.</div>
            </div>
          )}
          {groups.map((g) => (
            <div key={g.key} style={{ marginBottom: 6 }}>
              <div style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.06em', padding: '8px 12px 4px' }}>{g.label}</div>
              {g.items.map((it) => {
                idx++;
                ...
                return (
                  <div key={it.id} data-idx={myIdx} onMouseEnter={() => setSel(myIdx)} onClick={() => { it.run(); onClose(); }} style={{
                    ...
                    <div style={{ minWidth: 0, flex: 1 }}>
                      <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{it.title}</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '9px 16px', borderTop: '1px solid var(--border)', background: 'var(--bg-soft)', fontSize: 11, color: 'var(--text-subtle)' }}>
          ...
          <span style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6 }}>
            <span style={{ fontWeight: 500, color: 'var(--accent)' }}>LexIA</span> Buscar
          </span>
        </div>
      </div>
    </div>
