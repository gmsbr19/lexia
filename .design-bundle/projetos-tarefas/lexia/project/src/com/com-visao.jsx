            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8 }}>
              {[['Leads', cmInt(c.leads)], ['Conversões', cmInt(c.conversoes)], ['Investimento', c.investimento ? cmCompact(c.investimento) : '—']].map(([l, v]) => (
                <div key={l} style={{ background: 'var(--bg-soft)', border: '1px solid var(--border)', borderRadius: 9, padding: '8px 10px' }}>
                  <div style={{ fontSize: 10, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 500 }}>{l}</div>
    ...
    <div style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
      <CmFrame>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 20, gap: 16, flexWrap: 'wrap' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 23, fontWeight: 500, letterSpacing: '-0.025em', color: 'var(--text)' }}>Visão geral · {scopeLabel.title}</h1>
