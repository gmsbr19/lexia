    ...
    <div style={{ position: 'relative', display: 'inline-block' }}>
      ...
      {open && (<>
        ...
        <div className="card" style={{ position: 'absolute', left: 0, top: 28, zIndex: 41, minWidth: 178, padding: 6, boxShadow: 'var(--shadow-lg)' }}>
          <div style={{ fontSize: 10, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 500, padding: '5px 9px 3px' }}>Mover para</div>
...
const CmLeadRow = ({ l, campMap, selected, onSelect, onMove, onConvert, onLose, onEdit, onReopen }) => {
  ...
  return (
    <tr className="cm-row" style={{ borderTop: '1px solid var(--border)', background: selected ? 'var(--accent-soft)' : undefined }}>
      ...
      <td style={{ padding: '11px 14px', textAlign: 'right' }}><CmNum size={12.5} weight={500}>{cmCompact(l.valorEstimado)}</CmNum></td>
      <td style={{ padding: '11px 14px' }}><CmNum size={12} weight={500} color="var(--text-muted)">{cmDate(l.dataEntrada)}</CmNum></td>
      <td style={{ padding: '11px 14px' }}>
        {l.etapa === 'ganho'
          ? <div><div style={{ fontSize: 12, fontWeight: 500, color: 'var(--text)', whiteSpace: 'nowrap' }}>{l.cliente || l.nome}</div><div style={{ fontSize: 11, color: 'var(--cm-pos,#2E9E5B)', fontWeight: 500, fontFamily: 'var(--font-mono)' }}>{cmCompact(l.valorContratado)}</div></div>
          : l.etapa === 'perdido'
