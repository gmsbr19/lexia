...
const CRM_ENTRY = (typeof window !== 'undefined' && window.CRM_ENTRY) || 'crm';
const CRM_LS_TABS = CRM_ENTRY === 'processos' ? 'lexia-proc-workspace-v2' : 'lexia-crm-workspace-v2';

// ---- Tweaks: aparência + auras do painel da LexIA ----
...
const CrmModuleStub = ({ icon, label, sub }) => (
  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100%', padding: 40 }}>
    <div style={{ textAlign: 'center', maxWidth: 380 }}>
      ...
      <div style={{ fontSize: 18, fontWeight: 500, color: 'var(--text)', letterSpacing: '-0.02em' }}>{label}</div>
        ...
        {[{ p: 'clientes', i: 'users', t: 'Clientes', s: 'Gerencie contatos e leads' }, { p: 'casos', i: 'briefcase', t: 'Casos', s: 'Rateio e financeiro por caso' }, { p: 'contratos', i: 'receipt', t: 'Contratos', s: 'Honorários e recebimentos' }, { p: 'agenda', i: 'calendar', t: 'Agenda', s: 'Audiências, prazos e reuniões' }].map((x) => (
          <CrmRow key={x.p} onClick={() => nav.navPage(x.p)} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: 18 }} >
            <div className="card" style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 14, padding: 16 }}>
              ...
              <div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text)' }}>{x.t}</div><div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{x.s}</div></div>
...
const CrmPageView = ({ route, store, procStore, procMut, role, nav, mut, ui, onTab, onRoute, onNovoCliente, onAnonimizar }) => {
  switch (route.page) {
    case 'contencioso': return typeof ProcContenciosoPage !== 'undefined'
      ? <ProcContenciosoPage route={route} onRoute={onRoute} procStore={procStore} crmStore={store} role={role} nav={nav} procMut={procMut} />
      : <CrmModuleStub icon="scale" label="Contencioso" sub="Módulo de processos e prazos." />;
    ...
    case 'documentos': return <CrmDocumentosPage store={store} onOpenDoc={nav.openDocumento} onGerar={ui.openLexia} />;
    case 'documento': return <CrmDocumentoEditor store={store} docId={route.docId} nav={nav} panelOpen={ui.aiDock} panelMode={ui.panelMode} setPanelMode={ui.setPanelMode} />;
    case 'notificacoes': return <CrmNotificacoesPage />;
    case 'tarefas': return <CrmModuleStub icon="listChecks" label="Tarefas" sub="O quadro de tarefas (kanban e lista) já existe no app LexIA · Tarefas. Esta rodada de redesign cobre Clientes, Casos, Contratos, Agenda e Documentos." />;
...
// ---- default workspace (first run) ----
function crmDefaultWorkspace(store) {
  // entrada Contencioso: abre direto no Painel (cockpit), painel único
  if (CRM_ENTRY === 'processos') {
    return { panes: [crmMkPane([{ page: 'contencioso', sub: 'painel' }])], active: 0, split: 0.5 };
  }
...
function crmLoadWorkspace(store) {
  const docIds = new Set((store.documentos || []).map((d) => d.id));
  // rotas de documento geradas em runtime não sobrevivem ao reload (store não é persistido);
  // troca rotas órfãs pela biblioteca para nunca cair em "não encontrado".
  const sane = (route) => (route && route.page === 'documento' && !docIds.has(route.docId)) ? { page: 'documentos' } : route;
  try {
    ...
    if (raw) {
      ...
      if (p && Array.isArray(p.panes) && p.panes.length && p.panes.every((pane) => pane.tabs && pane.tabs.length)) {
        const panes = p.panes.map((pane) => ({ ...pane, tabs: pane.tabs.map((t) => ({ ...t, route: sane(t.route) })) }));
        return { active: 0, split: 0.5, ...p, panes, active: Math.min(p.active || 0, panes.length - 1) };
      }
    }
  } catch (e) { /* ignore */ }
...
const CrmApp = () => {
  ...
  const [procStore, setProcStore] = crmUseState(() => procBuildStore());
  ...
  const [barOpen, setBarOpen] = crmUseState(false);   // unified LexIA bar (busca + ação + IA)
  const [barSeed, setBarSeed] = crmUseState('');
  const [docPanelMode, setDocPanelMode] = crmUseState('chat'); // painel do documento: 'chat' | 'form'
  ...
  crmUseEffect(() => {
    const h = (e) => {
      if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'k') { e.preventDefault(); setBarSeed(''); setBarOpen((s) => !s); }
    };
  ...
  const navCurrent = {
    ...
    openDocumento: (id) => ctl.navCurrent({ page: 'documento', docId: id }),
    openProcesso: (id) => { const p = procById(procStore, id); ctl.navCurrent({ page: 'contencioso', sub: 'processos', fichaId: id, procNum: p ? p.numero.split('-')[0] : '' }); },
    openCaso: (id) => setCasoId(id),
    openContrato: (id) => setContratoId(id),
  };

  // ---- mutations do contencioso (prazos / triagem / anotações) ----
  const procMut = {
    salvarPrazo: (prazo, movId) => setProcStore((s) => ({
      ...s,
      prazos: [...s.prazos, prazo],
      andamentos: movId ? s.andamentos.map((a) => a.id === movId ? { ...a, triagem: 'relevante', lido: true } : a) : s.andamentos,
    })),
    triar: (movId, classif) => setProcStore((s) => ({ ...s, andamentos: s.andamentos.map((a) => a.id === movId ? { ...a, triagem: classif, lido: true } : a) })),
    protocolar: (prazoId) => setProcStore((s) => ({ ...s, prazos: s.prazos.map((p) => p.id === prazoId ? { ...p, status: 'protocolado' } : p) })),
    addNota: (procId, texto) => setProcStore((s) => ({ ...s, notas: [{ id: 'N' + Math.random().toString(36).slice(2, 7), processoId: procId, autor: 'thiago', data: PROC_TODAY, texto }, ...s.notas] })),
  };

  // ---- documentos: criação/geração pela LexIA ----
  const patchDoc = (id, patch) => setStore((s) => ({ ...s, documentos: s.documentos.map((d) => d.id === id ? { ...d, ...patch, data: CRM_TODAY } : d) }));
  const createDoc = (spec) => {
    const id = 'D' + Date.now().toString(36).toUpperCase();
    const cli = store.clientes.find((c) => c.id === spec.clienteId);
    const nome = spec.tipo + (cli ? ` — ${cli.apelido || cli.nome}` : '');
    setStore((s) => ({ ...s, documentos: [{
      id, nome, modelo: spec.tipo, formato: 'DOCX', data: CRM_TODAY, status: 'gerando',
      clienteId: spec.clienteId || null, autor: 'thiago', palavras: 0,
      numero: 'DOC-2026-0' + (110 + s.documentos.length), multa: false, prompt: spec.prompt,
    }, ...s.documentos] }));
    setDocPanelMode('chat'); // documento recém-gerado abre com o chat da LexIA
    ctl.navCurrent({ page: 'documento', docId: id });
    return id;
  };
  const finishDoc = (id) => patchDoc(id, { status: 'rascunho', palavras: 1184 + Math.floor(Math.random() * 260) });
  ...
  const renderPane = (pane, pi) => {
    ...
    const paneNav = {
      ...
      openDocumento: (id) => ctl.navInPane(pi, { page: 'documento', docId: id }),
      openProcesso: (id) => { const p = procById(procStore, id); ctl.navInPane(pi, { page: 'contencioso', sub: 'processos', fichaId: id, procNum: p ? p.numero.split('-')[0] : '' }); },
      openCaso: (id) => setCasoId(id),
      openContrato: (id) => setContratoId(id),
    };
    const ui = {
      aiDock: barOpen && route.page === 'documento',
      panelMode: docPanelMode,
      setPanelMode: (m) => { setBarOpen((w) => !(w && docPanelMode === m)); setDocPanelMode(m); },
      openLexia: (seedTxt) => { setBarSeed(seedTxt || ''); setBarOpen(true); },
    };
    return (
      <CrmPageView route={route} store={store} procStore={procStore} procMut={procMut} role={role} nav={paneNav} mut={mut} ui={ui}
        onTab={(t) => ctl.patchTab(pi, tab.id, { clienteTab: t })}
        onRoute={(patch) => ctl.patchTab(pi, tab.id, patch)}
        onNovoCliente={() => setQuick('cliente')}
        onAnonimizar={(id) => setAnon({ clienteId: id })} />
    );
  };

  const lexiaHandlers = { ...navCurrent, action: mut.action, createDoc, finishDoc, patchDoc };
  const lexiaCtx = crmUseMemo(() => ({
    page: activeRoute.page,
    cliente: activeRoute.page === 'cliente' ? store.clientes.find((x) => x.id === activeRoute.clienteId) : null,
    doc: activeRoute.page === 'documento' ? store.documentos.find((d) => d.id === activeRoute.docId) : null,
  }), [activeRoute, store]);
  const lexiaDock = activeRoute.page === 'documento';

  return (
    <div className={`lex-artboard theme-${theme}`}>
      <CrmToastHost>
        <CrmNtfProvider nav={(page) => ctl.navCurrent({ page })}>
        <div className="app-screen">
          <CrmShell ws={ws} ctl={ctl} store={store} role={role} renderPane={renderPane}
            ...
            onRoleChange={setRole} onOpenSpotlight={() => { setBarSeed(''); setBarOpen(true); }} onOpenSettings={() => setSettings(true)} />

          {/* Barra LexIA — superfície única de IA: dock central + comando + conversa; acopla ao documento */}
          <CrmLexiaBar open={barOpen} onOpenChange={setBarOpen} ctx={lexiaCtx} store={store} handlers={lexiaHandlers} seed={barSeed} dock={lexiaDock} dockMode={docPanelMode}
            tint={tw.panelTint} auraOpacity={tw.auraOpacity} auraSpread={tw.auraSpread} auraSpeed={tw.auraSpeed} auraColor={tw.auraColor} borderGlow={tw.borderGlow} borderSpeed={tw.borderSpeed} />
          ...
          <TweaksPanel title="Tweaks">
            <TweakSection label="Vidro" />
            <TweakSlider label="Opacidade do vidro" value={tw.panelTint} min={0} max={100} step={5} unit="%" onChange={(v) => setTweak('panelTint', v)} />
            <TweakSection label="Auras de fundo" />
            <TweakSlider label="Intensidade" value={tw.auraOpacity} min={0} max={150} step={5} unit="%" onChange={(v) => setTweak('auraOpacity', v)} />
            <TweakSlider label="Amplitude" value={tw.auraSpread} min={60} max={200} step={5} unit="%" onChange={(v) => setTweak('auraSpread', v)} />
            <TweakSlider label="Velocidade" value={tw.auraSpeed} min={1} max={10} step={1} onChange={(v) => setTweak('auraSpeed', v)} />
            <TweakRadio label="Cor" value={tw.auraColor} options={[{ value: 'dourado', label: 'Dourado' }, { value: 'azul', label: 'Azul' }, { value: 'misto', label: 'Misto' }]} onChange={(v) => setTweak('auraColor', v)} />
            <TweakSection label="Borda" />
            <TweakSlider label="Brilho da borda" value={tw.borderGlow} min={0} max={100} step={5} unit="%" onChange={(v) => setTweak('borderGlow', v)} />
            <TweakSlider label="Velocidade da borda" value={tw.borderSpeed} min={1} max={10} step={1} onChange={(v) => setTweak('borderSpeed', v)} />
          </TweaksPanel>
        </div>
        </CrmNtfProvider>
      </CrmToastHost>
