...
const FxStatusPill = ({ status }) => {
  ...
  return (
    <span style={{
      ...
      padding: '0 9px', height: 22, borderRadius: 6, background: t.bg, color: t.fg, whiteSpace: 'nowrap',
    }}>
...
const FxTabs = ({ tabs, active, onChange }) => (
  <div style={{
    display: 'flex', alignItems: 'stretch', gap: 2, padding: '0 22px',
    borderBottom: '1px solid var(--border)', background: 'var(--bg)', minHeight: 46, flexShrink: 0,
  }}>
    {tabs.map((t) => {
      ...
      return (
        <div key={t.id} onClick={() => onChange(t.id)} style={{
          ...
          borderBottom: on ? '2px solid var(--text)' : '2px solid transparent', marginBottom: -1,
          ...
          {t.badge != null && (
            <span style={{
              fontSize: 11, fontWeight: 500, background: on ? 'rgba(192,73,47,0.12)' : 'var(--bg-sunken)',
              color: on ? 'var(--fin-neg,#C0492F)' : 'var(--text-subtle)', padding: '1px 6px', borderRadius: 999, fontFeatureSettings: '"tnum"',
            }}>{t.badge}</span>
          )}
        </div>
      );
...
const FxInput = (props) => <input {...props} className="input" style={{ height: 38, fontSize: 14, ...(props.style || {}) }} />;
const FxSelect = ({ options, value, onChange, placeholder, ...rest }) => (
  <div style={{ position: 'relative' }}>
    <select value={value} onChange={onChange} {...rest} className="input" style={{
      height: 38, fontSize: 14, appearance: 'none', WebkitAppearance: 'none', paddingRight: 34, cursor: 'pointer',
    }}>
...
const FxKpi = ({ label, value, sub, icon, accent, tone }) => (
  <div className="card" style={{ padding: '15px 17px', display: 'flex', flexDirection: 'column', gap: 9, minHeight: 104 }}>
    ...
    <span style={{
      fontSize: 24, fontWeight: 500, letterSpacing: '-0.025em', fontFeatureSettings: '"tnum"', fontVariantNumeric: 'tabular-nums',
