      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: CRM_CLI_COLS, gap: 14, padding: '11px 18px', borderBottom: '1px solid var(--border)', background: 'var(--bg-soft)' }}>
          {['Cliente', 'Tipo', 'CPF/CNPJ', 'Cidade/UF', 'Classificação', 'Casos'].map((h, i) => (
            <div key={h} style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.08em', textAlign: i >= 5 ? 'right' : 'left' }}>{h}</div>
          ))}
        </div>
        {rows.length === 0 ? (
            ...
            <CrmRow key={c.id} onClick={() => onOpenCliente(c.id)} style={{ display: 'grid', gridTemplateColumns: CRM_CLI_COLS, gap: 14, padding: '12px 18px', alignItems: 'center', borderTop: i ? '1px solid var(--border)' : 'none' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 500, color: 'var(--text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.nome}</div>
                  {c.apelido && <div style={{ fontSize: 12, color: 'var(--text-subtle)' }}>{c.apelido}</div>}
                </div>
              </div>
              <div><CrmTipoBadge tipo={c.tipo} /></div>
              <div style={{ fontSize: 12, color: c.doc ? 'var(--text-muted)' : 'var(--text-subtle)', fontVariantNumeric: 'tabular-nums' }}>{c.doc || '—'}</div>
              <div style={{ fontSize: 12, color: c.cidade ? 'var(--text-muted)' : 'var(--text-subtle)' }}>{c.cidade ? `${c.cidade}/${c.uf}` : '—'}</div>
              <div><CrmClasseBadge classe={c.classe} /></div>
              <div style={{ textAlign: 'right', fontSize: 13, fontWeight: 500, color: casos ? 'var(--text)' : 'var(--text-subtle)', fontVariantNumeric: 'tabular-nums' }}>{casos}</div>
            </CrmRow>
          );
        })}
      </div>
      <div style={{ fontSize: 12, color: 'var(--text-subtle)', textAlign: 'center', marginTop: 14 }}>{rows.length} de {store.clientes.length} clientes</div>
    </FxFrame>
  );
};
