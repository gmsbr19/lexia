...
const CRM_PAGE_META = {
  contencioso: { icon: 'scale', label: 'Contencioso' },
...
function crmTabMeta(route, store) {
  if (route.page === 'contencioso') {
    return { icon: 'scale', label: route.fichaId && route.procNum ? ('Proc ' + route.procNum) : 'Contencioso' };
  }
  if (route.page === 'processo') {
    return { icon: 'scale', label: route.procNum ? ('Proc ' + route.procNum) : 'Processo' };
  }
  ...
  if (route.page === 'documento') {
    const d = (store.documentos || []).find((x) => x.id === route.docId);
    return { icon: 'fileText', label: d ? d.nome : 'Documento' };
  }
...
const CrmTabStrip = ({ pane, paneIdx, isActivePane, paneCount, store, role, ctl, leading, trailing }) => {
  ...
  const pages = [...(typeof CRM_NAV_CONTENCIOSO !== 'undefined' ? CRM_NAV_CONTENCIOSO : []), ...CRM_NAV].filter((n) => !(n.socioPlus && role === 'staff'));
    ...
    <div style={{ position: 'relative', display: 'flex', alignItems: 'center', gap: 8, padding: '9px 12px', minHeight: 51, background: 'var(--bg)', borderBottom: '1px solid var(--border)', flexShrink: 0 }}>
      ...
      {add && (
        <CrmFloatMenu x={add.x} y={add.y} width={220} onClose={() => setAdd(null)}>
          <div style={{ fontSize: 11, fontWeight: 500, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.06em', padding: '6px 10px 4px' }}>Abrir nova aba</div>
          {pages.map((n) => (
