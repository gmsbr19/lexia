...
const KpiCard = ({ label, value, delta, deltaDir, sub, icon, accent }) => (
  <div className="card" style={{ padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 10, minHeight: 112 }}>
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      <span style={{ fontSize: 12, color: 'var(--text-muted)', letterSpacing: '0.01em', fontWeight: 500 }}>{label}</span>
      {icon && (
        <div style={{
    ...
    <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, flexWrap: 'wrap' }}>
      <span style={{ fontSize: 25, fontWeight: 500, color: 'var(--text)', letterSpacing: '-0.025em', fontFeatureSettings: '"tnum"', fontFamily: 'var(--font-sans)' }}>{value}</span>
      {delta && (
        <span style={{
    ...
    {sub && <span style={{ fontSize: 12, color: 'var(--text-subtle)', marginTop: -2 }}>{sub}</span>}
  </div>
);
...
const StatusPill = ({ label, tone = 'neutro', dot = true }) => {
  ...
  return (
    <span style={{
      ...
      fontSize: 11, fontWeight: 500, padding: dot ? '3px 9px 3px 8px' : '3px 9px',
      borderRadius: 6, background: t.bg, color: t.fg, letterSpacing: 0, whiteSpace: 'nowrap',
    }}>
...
const Th = ({ children, align = 'left' }) => (
  <th style={{
    textAlign: align, padding: '10px 16px', fontSize: 11, fontWeight: 500,
...
const Td = ({ children, align = 'left', muted, subtle, style }) => (
  <td style={{
    padding: '12px 16px', fontSize: 12, textAlign: align,
...
const FooterBar = ({ showing, total, label = 'registros' }) => (
  <div style={{
    ...
    borderTop: '1px solid var(--border)', background: 'var(--bg-soft)', fontSize: 12, color: 'var(--text-muted)',
  }}>
...
const Avatar = ({ name, gold }) => (
  <div style={{
    ...
    background: gold ? 'var(--accent-soft)' : 'var(--bg-sunken)',
    color: gold ? 'var(--accent)' : 'var(--text)',
  ...
  return (
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)', letterSpacing: '-0.01em' }}>{title}</div>
