    ...
    <CmModal width={540} title="Converter lead" sub={`${lead.nome} · ${lead.origem}`} onClose={onClose}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 15 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '12px 14px', background: 'rgba(46,158,91,0.08)', border: '1px solid rgba(46,158,91,0.26)', borderRadius: 'var(--r-sm)' }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.5 }}>Marca o lead como <strong style={{ color: 'var(--text)', fontWeight: 500 }}>Ganho</strong> e cria o vínculo com cliente, caso e honorário no Financeiro.</div>
        </div>
      ...
      {stage === 'pick' ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, padding: '28px 20px', border: '1.5px dashed var(--border-strong)', borderRadius: 'var(--r-md)', background: 'var(--bg-soft)' }}>
            ...
            <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)' }}>genions-leads-junho.csv</div>
            <div style={{ fontSize: 12, color: 'var(--text-subtle)' }}>Arquivo de exemplo · arraste outro para substituir</div>
          </div>
          <div style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.5 }}>O Genions exporta nome, contato, origem e etapa. Leads já existentes (mesmo contato) são <strong style={{ color: 'var(--text)' }}>atualizados</strong>; novos são adicionados ao funil em <strong style={{ color: 'var(--text)' }}>Novo</strong>.</div>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '12px 14px', background: 'rgba(46,158,91,0.08)', border: '1px solid rgba(46,158,91,0.26)', borderRadius: 'var(--r-sm)' }}>
            <Icon name="checkCircle" size={16} style={{ color: '#2E9E5B', flexShrink: 0 }} />
            <div style={{ fontSize: 12, color: 'var(--text)' }}><strong style={{ fontWeight: 500 }}>{preview.total} registros</strong> lidos com sucesso, prontos para importar.</div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div style={{ background: 'var(--bg-soft)', border: '1px solid var(--border)', borderRadius: 10, padding: '13px 15px' }}>
              ...
              <div style={{ fontSize: 24, fontWeight: 500, color: 'var(--accent)', fontFamily: 'var(--font-mono)' }}>{preview.novos}</div>
            </div>
            <div style={{ background: 'var(--bg-soft)', border: '1px solid var(--border)', borderRadius: 10, padding: '13px 15px' }}>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 500 }}>Atualizados</div>
              <div style={{ fontSize: 24, fontWeight: 500, color: 'var(--text)', fontFamily: 'var(--font-mono)' }}>{preview.atualizados}</div>
            </div>
          </div>
        </div>
      )}
