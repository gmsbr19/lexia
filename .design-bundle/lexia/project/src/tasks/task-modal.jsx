...
const PickerButton = ({ children, muted }) => (
  <span className="picker-btn" style={{
    ...
    cursor: 'pointer', fontSize: 12.5, fontWeight: 500, color: muted ? 'var(--text-subtle)' : 'var(--text)',
  }}>{children}</span>
);
...
const CriteriaList = ({ title, sub, items, accent, onToggle, onGenerate, onRemove }) => (
  <div style={{ background: 'var(--bg-soft)', border: '1px solid var(--border)', borderRadius: 12, padding: '13px 14px' }}>
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: items.length ? 10 : 0 }}>
      <span style={{ fontSize: 11, fontWeight: 700, color: accent, letterSpacing: '0.06em', textTransform: 'uppercase' }}>{title}</span>
      <span style={{ fontSize: 11, color: 'var(--text-subtle)' }}>{sub}</span>
      <div style={{ flex: 1 }} />
      <button className="btn btn-ghost" onClick={onGenerate} style={{ height: 26, fontSize: 11.5, padding: '0 9px', color: 'var(--accent)' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
        {items.map((c, i) => (
          <div key={i} className="crit-row" style={{ display: 'flex', alignItems: 'flex-start', gap: 9 }}>
            ...
            <span style={{ flex: 1, fontSize: 12.5, lineHeight: 1.45, color: c.done ? 'var(--text-subtle)' : 'var(--text-muted)', textDecoration: c.done ? 'line-through' : 'none' }}>{c.text}</span>
    ...
    <div onClick={onClose} style={{
      ...
      <div onClick={(e) => e.stopPropagation()} className="card" style={{
        ...
        <div style={{ overflowY: 'auto', padding: '18px 22px 22px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 18 }}>
            ...
            <textarea
              ...
              style={{
                ...
                fontFamily: 'var(--font-sans)', fontSize: 19, fontWeight: 600, color: 'var(--text)',
          ...
          <div style={{ marginBottom: 18 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>Subtarefas</span>
              {subs.length > 0 && <span style={{ fontSize: 11.5, color: 'var(--text-subtle)' }}>{subsDone}/{subs.length}</span>}
            </div>
            {subs.length > 0 && (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 18 }}>
            <CriteriaList title="DoR" sub="pronto para começar" accent="#2A6FDB" items={task.dor || []}
          ...
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>Descrição</span>
            </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px', borderTop: '1px solid var(--border)', background: 'var(--bg-soft)' }}>
          <button onClick={() => { onDelete(task.id); onClose(); }} className="btn btn-ghost" style={{ height: 32, fontSize: 12.5, color: 'var(--fin-neg, #C0492F)' }}>
            <Icon name="trash2" size={14} strokeWidth={1.9} />Excluir
          </button>
          <div style={{ flex: 1 }} />
          <button onClick={onClose} className="btn btn-primary" style={{ height: 32, fontSize: 12.5 }}>Concluir edição</button>
        </div>
      </div>
    </div>
  );
