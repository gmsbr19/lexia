...
const CmExportItem = ({ icon, label, desc }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '9px 0' }}>
    ...
    <div><div style={{ fontSize: 12.5, fontWeight: 500, color: 'var(--text)' }}>{label}</div><div style={{ fontSize: 11, color: 'var(--text-subtle)' }}>{desc}</div></div>
  </div>
);
    ...
    <div style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
      <CmFrame>
        <div style={{ marginBottom: 22 }}>
          <h1 style={{ margin: 0, fontSize: 23, fontWeight: 600, letterSpacing: '-0.025em', color: 'var(--text)' }}>Exportar · {scopeLabel.title}</h1>
        ...
        <div className="cm-2col" style={{ display: 'grid', gridTemplateColumns: '1fr 1.25fr', gap: 20, alignItems: 'start' }}>
          ...
          <div className="card" style={{ padding: '20px 22px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--text)' }}>Relatório com IA</div>
                <div style={{ fontSize: 11.5, color: 'var(--text-subtle)' }}>Copie o prompt e cole no Claude com o CSV anexado</div>
              </div>
              <button className="btn btn-gold" onClick={copy} style={{ height: 32, fontSize: 12.5 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10, fontSize: 11.5, color: 'var(--text-muted)' }}>
              ...
              Dica: anexe o <strong style={{ color: 'var(--text)', fontWeight: 600 }}>CSV baixado ao lado</strong> antes de enviar — o relatório fica muito mais preciso.
            </div>
          </div>
        </div>
      </CmFrame>
