  ...
  return (
    <div style={{ width: '100%', height: H }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0 2px', marginTop: -22 }}>
        {data.map((d, i) => (
          <span key={i} style={{ fontSize: 10.5, color: d.proj ? 'var(--text-subtle)' : 'var(--text-muted)', fontWeight: d.proj ? 400 : 500, fontFeatureSettings: '"tnum"' }}>{d.mes}</span>
        ))}
      </div>
    </div>
  );
...
const ChartLegend = ({ items }) => (
  <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap' }}>
    {items.map((it, i) => (
      <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 11.5, color: 'var(--text-muted)' }}>
        <span style={{
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 22, flexWrap: 'wrap' }}>
      <div style={{ position: 'relative', width: size, height: size, flexShrink: 0 }}>
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          <span style={{ fontSize: 10.5, color: 'var(--text-subtle)', letterSpacing: '0.02em' }}>Total</span>
          <span style={{ fontSize: 17, fontWeight: 600, color: 'var(--text)', letterSpacing: '-0.02em', fontFeatureSettings: '"tnum"' }}>{brlCompact(total)}</span>
        </div>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 11, flex: 1, minWidth: 150 }}>
        {data.map((d, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ fontSize: 12.5, color: 'var(--text)', flex: 1 }}>{d.label}</span>
            <span style={{ fontSize: 11.5, color: 'var(--text-subtle)', fontFeatureSettings: '"tnum"' }}>{Math.round(d.value / total * 100)}%</span>
            <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)', fontFamily: 'var(--font-mono)', fontFeatureSettings: '"tnum"', minWidth: 78, textAlign: 'right' }}>{brlCompact(d.value)}</span>
          </div>
        ))}
      </div>
    </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      {data.map((d, i) => (
        <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <span style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--text)', fontFamily: 'var(--font-mono)', fontFeatureSettings: '"tnum"', width: 84, textAlign: 'right' }}>{brlCompact(d.value)}</span>
  ...
  return (
    <div>
      ...
      <div style={{ position: 'relative', height: 18, marginTop: 4 }}>
        <span style={{ position: 'absolute', left: `${bePct}%`, transform: 'translateX(-50%)', fontSize: 10.5, color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8 }}>
        ...
        <span style={{ fontSize: 12.5, fontWeight: 600, color: acima ? 'var(--fin-pos,#2E9E5B)' : 'var(--fin-neg,#C0492F)' }}>
