...
const StatCard = ({ label, value, trend, trendDir }) => (
  <div className="card" style={{ padding: '14px 16px' }}>
    <div style={{ fontSize: 11.5, color: 'var(--text-muted)', letterSpacing: '0.01em' }}>{label}</div>
    <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginTop: 5 }}>
      <span style={{ fontSize: 22, fontWeight: 600, color: 'var(--text)', letterSpacing: '-0.02em', fontFeatureSettings: '"tnum"' }}>{value}</span>
      {trend && <span style={{ fontSize: 11, fontWeight: 600, color: trendDir === 'down' ? 'var(--fin-neg,#C0492F)' : 'var(--fin-pos,#2E9E5B)' }}>{trend}</span>}
    </div>
  </div>
);
...
const ClientesScreen = () => (
  <AppShell
    active="clientes"
    breadcrumb={['Clientes']}
    actions={<button className="btn btn-primary" style={{ height: 32, fontSize: 12.5 }}><Icon name="plus" size={13} />Novo cliente</button>}
  >
    <PageFrame>
      ...
      <FilterBar>
        ...
        <div style={{ flex: 1 }} />
        <button className="btn btn-secondary" style={{ height: 32, fontSize: 12.5 }}><Icon name="filter" size={13} />Filtros</button>
      </FilterBar>
      <TableShell
...
const DocChip = ({ bucket }) => (
  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11, fontWeight: 500, color: 'var(--text-muted)', background: 'var(--bg-sunken)', padding: '3px 9px', borderRadius: 999 }}>{bucket}</span>
);
    ...
    <Td>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div style={{ width: 28, height: 36, borderRadius: 4, background: '#fff', border: '1px solid var(--border-strong)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', paddingBottom: 4, flexShrink: 0, position: 'relative' }}>
          <span style={{ fontSize: 7, fontWeight: 700, color: '#020D25', fontFamily: 'var(--font-mono)' }}>HN</span>
...
const ContratosScreen = () => (
  <AppShell
    ...
    actions={<button className="btn btn-primary" style={{ height: 32, fontSize: 12.5 }}><Icon name="plus" size={13} />Novo honorário</button>}
  >
    <PageFrame>
