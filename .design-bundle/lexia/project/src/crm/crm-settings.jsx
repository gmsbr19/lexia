...
const CRM_AUDIT = [
  { who: 'Thiago Moraes', when: '11/06 14:22', acao: 'Atualizou rateio', ent: 'Caso · Cavanhas' },
  { who: 'Leonardo Prado', when: '11/06 11:05', acao: 'Marcou como recebido', ent: 'Contrato H006' },
  { who: 'Marina Castro', when: '10/06 17:48', acao: 'Criou tarefa', ent: 'Tarefa T010' },
  { who: 'Leandro Moraes', when: '10/06 09:31', acao: 'Editou cliente', ent: 'Construtora Aurora' },
  { who: 'Thiago Moraes', when: '09/06 16:12', acao: 'Anonimizou cliente (LGPD)', ent: 'Cliente C021' },
];
const CRM_CUSTOS = [
...
const CrmSettings = ({ role, theme, onSetTheme, onClose, onAnonimizar }) => {
  ...
  return (
    <div onMouseDown={onClose} style={{ position: 'absolute', inset: 0, zIndex: 90, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(2,13,37,0.42)', backdropFilter: 'blur(4px)', padding: 24 }}>
      <div onMouseDown={(e) => e.stopPropagation()} className="crm-pop-in" style={{ width: 820, maxWidth: '100%', height: 580, maxHeight: '92%', display: 'flex', background: 'var(--surface)', border: '1px solid var(--border-strong)', borderRadius: 18, boxShadow: 'var(--shadow-lg)', overflow: 'hidden' }}>
        {/* nav */}
        <div style={{ width: 232, flexShrink: 0, background: 'var(--bg-soft)', borderRight: '1px solid var(--border)', padding: '18px 12px', display: 'flex', flexDirection: 'column' }}>
          <div style={{ fontSize: 16, fontWeight: 600, letterSpacing: '-0.02em', color: 'var(--text)', padding: '0 8px 14px' }}>Configurações</div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 22px', borderBottom: '1px solid var(--border)' }}>
            <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--text)', letterSpacing: '-0.01em' }}>{visible.find((v) => v.id === sec)?.label}</div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '22px' }}>
            {sec === 'perfil' && (
              <div style={{ maxWidth: 420 }}>
                <CrmField label="Nome"><FxInput defaultValue="Thiago Moraes" /></CrmField>
                <CrmField label="E-mail"><FxInput defaultValue="thiago@moraesadv.com.br" /></CrmField>
                  ...
                  {[{ id: 'light', label: 'Claro', icon: 'sun' }, { id: 'dark', label: 'Escuro', icon: 'moon' }, { id: 'system', label: 'Sistema', icon: 'sliders' }].map((t) => {
                    ...
                    return (
                      <button key={t.id} onClick={() => { if (t.id !== 'system') onSetTheme(t.id); toast(`Tema: ${t.label}`); }} style={{
                        ...
                        <Icon name={t.icon} size={20} /><span style={{ fontSize: 12.5, fontWeight: 600 }}>{t.label}</span>
                      </button>
                    );
                  })}
                </div>
            ...
            {sec === 'usuarios' && (
              <div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                  <div style={{ fontSize: 12.5, color: 'var(--text-muted)' }}>{CRM_USERS.length} usuários</div>
                  <button className="btn btn-primary" onClick={() => toast('Convite enviado')} style={{ height: 32, fontSize: 12.5 }}><Icon name="userPlus" size={14} />Novo usuário</button>
                </div>
                  ...
                  {CRM_USERS.map((u, i) => (
                    <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px', borderTop: i ? '1px solid var(--border)' : 'none' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 11.5, color: 'var(--text-subtle)' }}>{u.email}</div>
                      </div>
            ...
            {sec === 'financeiro' && (
              <div>
                <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', marginBottom: 4 }}>Custos fixos & pró-labore</div>
                  ...
                  {CRM_CUSTOS.map((c, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px', borderTop: i ? '1px solid var(--border)' : 'none' }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 11.5, color: 'var(--text-subtle)' }}>{c.cat} · dia {c.dia} · {c.conta}</div>
                      </div>
                      <span style={{ fontSize: 13, fontWeight: 600, fontVariantNumeric: 'tabular-nums', color: 'var(--text)' }}>{fxMoney(c.valor)}</span>
                ...
                <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', margin: '24px 0 12px' }}>Contas</div>
                <div className="card" style={{ overflow: 'hidden' }}>
                  {CRM_CONTAS.map((c, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px', borderTop: i ? '1px solid var(--border)' : 'none' }}>
            ...
            {sec === 'escritorio' && (
              <div style={{ maxWidth: 460 }}>
                <CrmField label="Razão social"><FxInput defaultValue="Moraes & Prado Advogados Associados" /></CrmField>
                ...
                <CrmField label="Timbrado / logotipo">
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px', border: '1px dashed var(--border-strong)', borderRadius: 12, color: 'var(--text-muted)' }}>
                    <Icon name="upload" size={18} /><span style={{ fontSize: 12.5 }}>Arraste o logotipo (PNG/SVG) ou clique para enviar</span>
                  </div>
                </CrmField>
              ...
              <div style={{ maxWidth: 520, display: 'flex', flexDirection: 'column', gap: 14 }}>
                {[{ t: 'Backup Astrea', s: 'Última importação: 09/06/2026 · 253 clientes, 129 casos', ic: 'refreshCw', btn: 'Reimportar' }, { t: 'Leads · CSV Genions', s: 'Última importação: 02/06/2026 · 14 leads', ic: 'upload', btn: 'Importar CSV' }].map((x, i) => (
                  <div key={i} className="card" style={{ padding: 16, display: 'flex', alignItems: 'center', gap: 14 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--text)' }}>{x.t}</div>
                      <div style={{ fontSize: 11.5, color: 'var(--text-subtle)', marginTop: 2 }}>{x.s}</div>
                    </div>
            ...
            {sec === 'lgpd' && (
              <div>
                <div className="card" style={{ padding: 16, marginBottom: 18, display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--text)' }}>Anonimização de cliente (LGPD)</div>
                    <div style={{ fontSize: 11.5, color: 'var(--text-subtle)', marginTop: 2 }}>Apaga dados pessoais mantendo o histórico financeiro. Ação irreversível.</div>
                  </div>
                  <button className="btn btn-secondary" onClick={() => { onClose(); onAnonimizar(); }} style={{ height: 32 }}>Selecionar cliente</button>
                </div>
                <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', marginBottom: 10 }}>Trilha de auditoria</div>
                <div className="card" style={{ overflow: 'hidden' }}>
                  {CRM_AUDIT.map((a, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', borderTop: i ? '1px solid var(--border)' : 'none' }}>
                      <div style={{ fontSize: 11.5, color: 'var(--text-subtle)', width: 78, flexShrink: 0, fontVariantNumeric: 'tabular-nums' }}>{a.when}</div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 12.5, color: 'var(--text)' }}><strong style={{ fontWeight: 600 }}>{a.who}</strong> · {a.acao}</div>
