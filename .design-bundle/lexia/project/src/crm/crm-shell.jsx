...
const CrmNavItem = ({ icon, label, active, badge, collapsed, onClick }) => (
  <button onClick={onClick} title={collapsed ? label : undefined} style={{
    ...
    padding: collapsed ? '9px' : '8px 10px', borderRadius: 9, border: 'none', cursor: 'pointer',
    background: active ? 'var(--accent-soft)' : 'transparent',
    color: active ? 'var(--accent)' : 'var(--text-muted)',
    fontSize: 13.5, fontWeight: active ? 600 : 500, letterSpacing: '-0.01em',
...
const CRM_NAV = [
  ...
  { id: 'documentos', icon: 'fileText', label: 'Documentos', badge: 12 },
  ...
  <aside style={{
    ...
    {!collapsed ? (
      <button onClick={onOpenSpotlight} style={{
        position: 'relative', marginBottom: 10, width: '100%', height: 34, borderRadius: 9,
        ...
        fontSize: 12.5, fontFamily: 'var(--font-sans)', textAlign: 'left',
      }}>
        <span style={{ position: 'absolute', left: 10, color: 'var(--text-subtle)' }}><Icon name="search" size={14} /></span>
        Buscar…
        <span style={{ marginLeft: 'auto', fontSize: 10.5, color: 'var(--text-subtle)', background: 'var(--bg-sunken)', padding: '2px 6px', borderRadius: 5, fontFamily: 'var(--font-mono)' }}>⌘K</span>
      </button>
    ) : (
      <button onClick={onOpenSpotlight} title="Buscar" style={{ marginBottom: 8, width: 40, height: 36, margin: '0 auto 8px', borderRadius: 9, border: '1px solid var(--border-strong)', background: 'var(--surface)', color: 'var(--text-muted)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Icon name="search" size={15} /></button>
    )}
    ...
    {!collapsed && (
      <div style={{ marginTop: 8, padding: '8px 10px', fontSize: 10.5, lineHeight: 1.5, color: 'var(--text-subtle)', background: 'var(--bg-sunken)', borderRadius: 8 }}>
  ...
  return (
    <div style={{ position: 'relative' }}>
      <button onClick={() => setOpen((o) => !o)} title="Trocar papel (demonstração de permissões)" style={{
        display: 'flex', alignItems: 'center', gap: 7, height: 32, padding: '0 8px 0 11px', borderRadius: 9,
        border: '1px solid var(--border-strong)', background: 'var(--surface)', cursor: 'pointer',
      ...
      {open && (
        <>
          ...
          <div className="card" style={{ position: 'absolute', top: 38, right: 0, zIndex: 41, width: 220, padding: 6, boxShadow: 'var(--shadow-lg)' }}>
            <div style={{ fontSize: 10.5, fontWeight: 600, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.06em', padding: '6px 10px 4px' }}>Papel · demonstração</div>
            {roles.map((r) => (
...
const CrmIconBtn = ({ icon, title, onClick, dot }) => (
  <button onClick={onClick} title={title} style={{ width: 34, height: 34, borderRadius: 9, border: '1px solid var(--border-strong)', background: 'var(--surface)', color: 'var(--text-muted)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', flexShrink: 0 }}>
...
const CrmShell = ({ ws, ctl, store, role, renderPane, activePage, onSidebarNav, collapsed, onToggleSidebar, theme, onToggleTheme, onRoleChange, onOpenSpotlight, onOpenSettings }) => {
  ...
  const trailing = (
    <div style={{ display: 'flex', alignItems: 'center', gap: 9, paddingLeft: 4 }}>
      ...
      <CrmIconBtn icon="bell" title="Notificações" dot />
      <div style={{ display: 'flex', alignItems: 'center', gap: 9, paddingLeft: 4 }}>
        <div style={{ width: 30, height: 30, borderRadius: '50%', background: 'linear-gradient(135deg, #020D25, #1a2c5a)', color: '#C0A147', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11.5, fontWeight: 600, flexShrink: 0 }}>{crmUser('thiago').iniciais}</div>
      </div>
    </div>
  );
  return (
...
Object.assign(window, { CrmShell, CrmLexiaOrb, CrmLogo, CRM_NAV });
