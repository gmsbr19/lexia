      ...
      <div onMouseDown={(e) => e.stopPropagation()} className="crm-pop-in card" style={{ width: 640, maxWidth: '100%', maxHeight: '92%', display: 'flex', flexDirection: 'column', boxShadow: 'var(--shadow-lg)', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '18px 22px', borderBottom: '1px solid var(--border)' }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ fontSize: 18, fontWeight: 600, letterSpacing: '-0.02em', color: 'var(--text)' }}>{h.descricao}</div>
            <div style={{ fontSize: 12.5, color: 'var(--text-muted)', marginTop: 4, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        ...
        <div style={{ overflowY: 'auto', padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div>
            <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)', marginBottom: 10 }}>{h.count > 1 ? 'Parcelas' : 'Lançamento'}</div>
            <div className="card" style={{ overflow: 'hidden' }}>
              {parcelas.map((l, i) => (
                <div key={l.id} className="crm-row" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px', borderTop: i ? '1px solid var(--border)' : 'none' }}>
                  <div style={{ width: 26, fontSize: 11.5, color: 'var(--text-subtle)', fontVariantNumeric: 'tabular-nums', flexShrink: 0 }}>{h.count > 1 ? `${i + 1}/${h.count}` : ''}</div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12.5, color: 'var(--text)', fontWeight: 500 }}>Venc. {fxDate(l.venc)}</div>
                  ...
                  <span style={{ width: 92, textAlign: 'right', fontSize: 13, fontWeight: 600, color: 'var(--text)', fontVariantNumeric: 'tabular-nums' }}>{fxMoney(l.valor)}</span>
                  {l.status === 'recebido'
                    ? <button onClick={() => mut.toggleRecebido(l.id)} className="btn btn-ghost" style={{ height: 28, fontSize: 11.5, padding: '0 9px', color: 'var(--text-muted)' }}>Desfazer</button>
      ...
      <div className="card" style={{ overflow: 'hidden' }}>
        <div style={{ display: 'grid', gridTemplateColumns: CRM_CON_COLS, gap: 14, padding: '11px 18px', borderBottom: '1px solid var(--border)', background: 'var(--bg-soft)' }}>
          {['Descrição', 'Cliente', 'Caso', 'Vencimento', 'Valor', 'Status'].map((hd, i) => (
            <div key={hd} style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.05em', textAlign: i === 4 ? 'right' : 'left' }}>{hd}</div>
          ))}
        </div>
        {rows.length === 0 ? <CrmEmpty icon="receipt" title="Nenhum contrato encontrado" sub="Ajuste a busca ou os filtros." /> : rows.slice(0, 60).map((l, i) => {
          ...
          return (
            <CrmRow key={l.id} onClick={() => onOpenContrato(l.contratoId)} style={{ display: 'grid', gridTemplateColumns: CRM_CON_COLS, gap: 14, padding: '12px 18px', alignItems: 'center', borderTop: i ? '1px solid var(--border)' : 'none' }}>
              ...
              <div style={{ fontSize: 12.5, color: 'var(--text-muted)', minWidth: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{cli ? (cli.apelido || cli.nome) : '—'}</div>
              <div style={{ fontSize: 12.5, color: 'var(--text-muted)', minWidth: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{caso ? caso.titulo : '—'}</div>
              <div style={{ fontSize: 12.5, color: 'var(--text-muted)', fontVariantNumeric: 'tabular-nums' }}>{fxDate(l.venc)}</div>
              <div style={{ textAlign: 'right', fontSize: 13, fontWeight: 600, color: 'var(--text)', fontVariantNumeric: 'tabular-nums' }}>{fxMoney(l.valor)}</div>
              <div><CrmContratoStatus status={l.status} venc={l.venc} /></div>
            </CrmRow>
          );
        })}
      </div>
      <div style={{ fontSize: 11.5, color: 'var(--text-subtle)', textAlign: 'center', marginTop: 14 }}>{Math.min(rows.length, 60)} de {rows.length} honorários</div>
    </FxFrame>
  );
};
