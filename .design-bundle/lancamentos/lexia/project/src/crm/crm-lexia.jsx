...
const CrmLexiaMsg = ({ role, children }) => (
  <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', flexDirection: role === 'user' ? 'row-reverse' : 'row' }}>
    {role === 'ai' && (
      <div style={{ width: 28, height: 28, borderRadius: 9, flexShrink: 0, background: 'linear-gradient(140deg,#d8be7a,#9a7f2e)', color: '#020D25', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.35)' }}>
        <Icon name="sparkles" size={15} />
      </div>
    )}
    <div style={{
      maxWidth: '82%', padding: '10px 13px', borderRadius: 14, fontSize: 13.5, lineHeight: 1.5, letterSpacing: '-0.01em',
...
const CrmLexiaCard = ({ card, handlers, onClose }) => {
  const tones = { neg: 'var(--fin-neg,#C0492F)', gold: 'var(--accent)', blue: 'var(--crm-blue,#2A6FB0)', pos: 'var(--fin-pos,#2E9E5B)' };
  ...
  return (
    <div style={{ marginLeft: 38, marginTop: 8, display: 'flex', alignItems: 'center', gap: 11, padding: '10px 12px', borderRadius: 12, border: '1px solid var(--border)', background: 'var(--surface)', boxShadow: 'var(--shadow-sm)' }}>
      ...
      <div style={{ minWidth: 0, flex: 1 }}>
        <div style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{card.title}</div>
        <div style={{ fontSize: 11, color: 'var(--text-subtle)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{card.sub}</div>
      </div>
      <button onClick={() => { card.run(handlers); onClose(); }} className="btn btn-secondary" style={{ height: 28, fontSize: 11.5, padding: '0 10px', flexShrink: 0 }}>{card.action}</button>
    </div>
  );
};
...
const CrmLexia = ({ ctx, store, onClose, handlers }) => {
  ...
  return (
    <div style={{ position: 'absolute', bottom: 96, right: 26, zIndex: 130, width: 430, maxWidth: 'calc(100% - 40px)', height: 600, maxHeight: 'calc(100% - 130px)', display: 'flex', flexDirection: 'column' }} className="crm-lexia-in">
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', background: 'var(--surface)', border: '1px solid var(--border-strong)', borderRadius: 20, boxShadow: '0 28px 70px rgba(2,13,37,0.34), 0 8px 22px rgba(2,13,37,0.18)', overflow: 'hidden' }}>
        {/* header */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '14px 16px', borderBottom: '1px solid var(--border)', background: 'var(--bg-soft)' }}>
          <div style={{ width: 34, height: 34, borderRadius: 11, background: 'linear-gradient(140deg,#d8be7a,#9a7f2e)', color: '#020D25', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.35)' }}>
            <Icon name="sparkles" size={18} />
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 14.5, fontWeight: 600, color: 'var(--text)', letterSpacing: '-0.02em' }}>LexIA</div>
        ...
        {showHist ? (
          <div style={{ flex: 1, overflowY: 'auto', padding: 12 }}>
            ...
            <div style={{ fontSize: 10.5, fontWeight: 600, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.06em', padding: '6px 6px 8px' }}>Recentes</div>
            {histItems.map((h, i) => (
          ...
          <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: '18px 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            {thread.length === 0 && (
              <div style={{ margin: 'auto 0' }}>
                <div style={{ textAlign: 'center', padding: '8px 0 22px' }}>
                  <div style={{ width: 52, height: 52, borderRadius: 16, margin: '0 auto 14px', background: 'linear-gradient(140deg,#d8be7a,#9a7f2e)', color: '#020D25', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 8px 24px rgba(192,161,71,0.35), inset 0 1px 0 rgba(255,255,255,0.4)' }} className="crm-orb-float">
                    <Icon name="sparkles" size={24} />
                  </div>
                  <div style={{ fontSize: 17, fontWeight: 600, color: 'var(--text)', letterSpacing: '-0.02em' }}>Olá, Thiago 👋</div>
            ...
            {thinking && (
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                <div style={{ width: 28, height: 28, borderRadius: 9, flexShrink: 0, background: 'linear-gradient(140deg,#d8be7a,#9a7f2e)', color: '#020D25', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Icon name="sparkles" size={15} /></div>
        ...
        {!showHist && (
          <div style={{ padding: '12px 14px', borderTop: '1px solid var(--border)', background: 'var(--bg-soft)' }}>
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, background: 'var(--surface)', border: '1px solid var(--border-strong)', borderRadius: 14, padding: '6px 6px 6px 14px' }}>
              <textarea value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); send(); } }}
                rows={1} placeholder="Pergunte à LexIA…" style={{
                  ...
                  fontSize: 13.5, color: 'var(--text)', maxHeight: 90, padding: '7px 0', letterSpacing: '-0.01em',
                }} />
              <button onClick={() => send()} disabled={!input.trim()} style={{
                width: 34, height: 34, borderRadius: 10, border: 'none', flexShrink: 0, cursor: input.trim() ? 'pointer' : 'default',
                background: input.trim() ? 'linear-gradient(140deg,#d8be7a,#9a7f2e)' : 'var(--bg-sunken)',
            ...
            <div style={{ fontSize: 10.5, color: 'var(--text-subtle)', textAlign: 'center', marginTop: 7 }}>A LexIA pode cometer erros. Confira informações importantes.</div>
          </div>
        )}
      </div>
    </div>
