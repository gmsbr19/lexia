...
const CrmEvtChip = ({ e, onClick, compact }) => {
  ...
  return (
    <button onClick={(ev) => { ev.stopPropagation(); onClick(e); }} style={{
      ...
      fontSize: 11, fontWeight: 600, letterSpacing: '-0.01em', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis',
  ...
  return (
    <FxModal title={evento && evento.id ? 'Editar evento' : 'Novo evento'} sub="Audiência, prazo, reunião ou outro" onClose={onClose} width={520}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: 9, fontSize: 12.5, color: 'var(--text-muted)', cursor: 'pointer' }}>
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)' }}>
          {CRM_WD.map((w) => <div key={w} style={{ padding: '8px 10px', fontSize: 11, fontWeight: 600, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{w}</div>)}
        </div>
          ...
          {cells.map((d, i) => {
            ...
            return (
              <div key={i} onClick={() => newAt(iso)} style={{ borderRight: (i % 7 !== 6) ? '1px solid var(--border)' : 'none', borderBottom: i < 35 ? '1px solid var(--border)' : 'none', padding: 6, minHeight: 92, background: inMonth ? 'transparent' : 'var(--bg-soft)', cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: 3, overflow: 'hidden' }}>
                ...
                {tks.slice(0, 1).map((t) => (
                  <div key={t.id} onClick={(ev) => { ev.stopPropagation(); nav.navPage('tarefas'); }} style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10.5, color: 'var(--text-muted)', padding: '1px 4px' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, border: '1px solid var(--border)', borderRadius: 12, overflow: 'hidden', background: 'var(--surface)' }}>
          ...
          {days.map((d, i) => {
            ...
            return (
              <div key={i} style={{ borderLeft: '1px solid var(--border)', padding: 6, minHeight: 38, display: 'flex', flexDirection: 'column', gap: 3 }}>
                <div style={{ fontSize: 11, fontWeight: 600, color: isToday ? 'var(--accent)' : 'var(--text-muted)', marginBottom: 2 }}>{CRM_WD[d.getDay()]} {d.getDate()}{isToday ? ' · hoje' : ''}</div>
                {allday.map((x) => x.tipo ? <CrmEvtChip key={x.id} e={x} onClick={openEvt} compact /> : (
                  <div key={x.id} onClick={() => nav.navPage('tarefas')} style={{ fontSize: 10.5, color: 'var(--text-muted)', display: 'flex', gap: 4, alignItems: 'center', cursor: 'pointer' }}><Icon name="flag" size={10} style={{ color: CRM_EVT.prazo.color }} />{x.titulo}</div>
                ))}
              </div>
            );
          })}
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: `52px repeat(${days.length},1fr)`, position: 'relative' }}>
            <div>
              {CRM_HOURS.map((h) => <div key={h} style={{ height: HOUR_PX, fontSize: 10.5, color: 'var(--text-subtle)', textAlign: 'right', paddingRight: 7, transform: 'translateY(-6px)' }}>{h}:00</div>)}
            </div>
            {days.map((d, ci) => {
              const iso = crmISO(d);
                  ...
                  {timed.map((e) => {
                    ...
                    return (
                      <div key={e.id}
                        ...
                        <div style={{ fontSize: 11, fontWeight: 600, color: m.color, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{e.titulo}</div>
          ...
          return (
            <div key={iso}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 9 }}>
                <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)', letterSpacing: '-0.01em' }}>{isToday ? 'Hoje' : CRM_WD_FULL[d.getDay()]}</span>
                ...
                {byDay[iso].map((e, i) => {
                  if (e._task) return (
                    <div key={e.id} onClick={() => nav.navPage('tarefas')} className="crm-row" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderTop: i ? '1px solid var(--border)' : 'none', cursor: 'pointer' }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)' }}>{e.titulo}</div><div style={{ fontSize: 11.5, color: 'var(--text-subtle)' }}>Tarefa · prazo · {crmFirst(e.responsavel)}</div></div>
                  ...
                  return (
                    <div key={e.id} onClick={() => openEvt(e)} className="crm-row" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderTop: i ? '1px solid var(--border)' : 'none', cursor: 'pointer' }}>
                      <div style={{ width: 52, flexShrink: 0, fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', fontVariantNumeric: 'tabular-nums' }}>{e.allDay ? 'dia' : e.hIni}</div>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}><div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text)' }}>{e.titulo}</div><div style={{ fontSize: 11.5, color: 'var(--text-subtle)' }}>{m.label}{e.local ? ' · ' + e.local : ''} · {crmFirst(e.responsavel)}</div></div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
      <div style={{ padding: '20px 32px 0', display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
          <h1 style={{ margin: 0, fontSize: 24, fontWeight: 600, letterSpacing: '-0.03em', color: 'var(--text)' }}>Agenda</h1>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <div style={{ minWidth: 180, textAlign: 'center', fontSize: 14.5, fontWeight: 600, color: 'var(--text)', letterSpacing: '-0.01em', textTransform: 'capitalize' }}>{title}</div>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              {['todos', 'audiência', 'prazo', 'reunião', 'outro'].map((t) => {
                ...
                return <button key={t} onClick={() => setFTipo(t)} style={{ height: 32, padding: '0 11px', borderRadius: 8, cursor: 'pointer', fontFamily: 'var(--font-sans)', fontSize: 12, fontWeight: 600, border: `1px solid ${on ? (m ? m.color : 'var(--accent)') : 'var(--border-strong)'}`, background: on ? (m ? m.soft : 'var(--accent-soft)') : 'var(--surface)', color: on ? (m ? m.color : 'var(--accent)') : 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 5 }}>
