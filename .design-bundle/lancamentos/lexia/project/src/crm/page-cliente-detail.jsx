...
const CrmStat = ({ label, value, tone }) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
    ...
    <span style={{ fontSize: 18, fontWeight: 600, letterSpacing: '-0.02em', fontVariantNumeric: 'tabular-nums', color: tone === 'neg' ? 'var(--fin-neg,#C0492F)' : tone === 'pos' ? 'var(--fin-pos,#2E9E5B)' : 'var(--text)' }}>{value}</span>
  </div>
);
  ...
  return (
    <div className="crm-row" style={{ display: 'grid', gridTemplateColumns: '24px 1fr 150px 110px 96px 120px', gap: 12, alignItems: 'center', padding: '11px 16px', borderTop: last ? 'none' : '1px solid var(--border)' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
        {l.status === 'recebido'
          ? <button onClick={() => onToggle(l.id)} className="btn btn-ghost" style={{ height: 26, fontSize: 11.5, padding: '0 9px', color: 'var(--fin-pos,#2E9E5B)' }}><Icon name="checkCircle" size={13} />Recebido</button>
      ...
      <div style={{ padding: '24px 40px 0', maxWidth: 1240, margin: '0 auto', width: '100%' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 18, flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 240 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
              <h1 style={{ margin: 0, fontSize: 24, fontWeight: 600, letterSpacing: '-0.03em', color: 'var(--text)' }}>{c.nome}</h1>
        ...
        {edit && (
          <div className="card" style={{ padding: 18, marginBottom: 18, background: 'var(--bg-soft)' }}>
            <div style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--text)', marginBottom: 14 }}>Editar dados cadastrais</div>
              ...
              sectionCard(tarefas.map((t, i) => (
                <div key={t.id} className="crm-row" style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderTop: i ? '1px solid var(--border)' : 'none' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11.5, color: 'var(--text-subtle)' }}>Prazo {fxDate(t.prazo)} · {crmFirst(t.responsavel)}</div>
                  </div>
                ...
                return (
                  <CrmRow key={k.id} onClick={() => nav.openCaso(k.id)} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 16px', borderTop: i ? '1px solid var(--border)' : 'none' }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11.5, color: 'var(--text-subtle)', marginTop: 2 }}>resp. {crmFirst(k.responsavel)} · {k.rateio.leandro}/{k.rateio.leonardo}</div>
                    </div>
                    <CrmCasoTipoPill tipo={k.tipo} />
                    <span style={{ fontSize: 13, fontWeight: 600, fontVariantNumeric: 'tabular-nums', color: 'var(--text)', width: 110, textAlign: 'right' }}>{fxMoney(kf.total)}</span>
                ...
                return (
                  <CrmRow key={h.id} onClick={() => nav.openContrato(h.id)} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 16px', borderTop: i ? '1px solid var(--border)' : 'none' }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11.5, color: 'var(--text-subtle)', marginTop: 2 }}>{h.tipo} · {recebido}/{h.count} parcelas recebidas</div>
                    </div>
                    <CrmBadge tone="neutral">{h.tipo}</CrmBadge>
                    <span style={{ fontSize: 13, fontWeight: 600, fontVariantNumeric: 'tabular-nums', color: 'var(--text)', width: 110, textAlign: 'right' }}>{fxMoney(h.valor)}</span>
                    <Icon name="chevronRight" size={16} style={{ color: 'var(--text-subtle)' }} />
                  </CrmRow>
                );
              }))}
                ...
                return (
                  <CrmRow key={e.id} onClick={() => nav.navPage('agenda')} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 16px', borderTop: i ? '1px solid var(--border)' : 'none' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 11.5, color: 'var(--text-subtle)' }}>{fxDateLong(e.dia)}{e.hIni ? ` · ${e.hIni}` : ''} · {crmFirst(e.responsavel)}</div>
                    </div>
              ...
              sectionCard(docs.map((d, i) => (
                <div key={d.id} className="crm-row" style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 16px', borderTop: i ? '1px solid var(--border)' : 'none' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11.5, color: 'var(--text-subtle)' }}>{d.modelo} · {d.formato} · {fxDate(d.data)}</div>
                  </div>
