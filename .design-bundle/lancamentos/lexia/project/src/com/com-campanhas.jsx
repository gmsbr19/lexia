...
const CmCampRow = ({ c, onGasto, onLeads, onEdit }) => {
  ...
  return (
    <tr className="cm-row" style={{ borderTop: '1px solid var(--border)' }}>
      ...
      <td style={{ padding: '12px 14px', textAlign: 'right' }}><CmNum size={12.5} weight={600} color="var(--text-muted)">{c.investimento ? cmCompact(c.investimento) : '—'}</CmNum></td>
    ...
    <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' }}>
      <CmFrame pad="22px 40px 12px">
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16, marginBottom: 16, flexWrap: 'wrap' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 23, fontWeight: 600, letterSpacing: '-0.025em', color: 'var(--text)' }}>Campanhas · {scopeLabel.title}</h1>
              ...
              {rows.length > 0 && (
                <tfoot>
                  <tr style={{ borderTop: '2px solid var(--border-strong)', background: 'var(--bg-soft)' }}>
                    ...
                    <td></td>
                    <td style={{ padding: '12px 14px', textAlign: 'right' }}><CmNum size={12.5} weight={700}>{cmCompact(tot.inv)}</CmNum></td>
                    <td style={{ padding: '12px 14px', textAlign: 'right' }}><CmNum size={12.5} weight={700}>{cmInt(tot.leads)}</CmNum></td>
                    <td style={{ padding: '12px 14px', textAlign: 'right' }}><CmNum size={12.5} weight={700}>{cmInt(tot.conv)}</CmNum></td>
                    <td style={{ padding: '12px 14px', textAlign: 'right' }}><CmNum size={12.5} weight={700} color="var(--cm-pos,#2E9E5B)">{cmCompact(tot.val)}</CmNum></td>
                    <td></td><td></td>
                    <td style={{ padding: '12px 14px', textAlign: 'right' }}><CmNum size={13} weight={700} color={totRoas >= 1 ? 'var(--cm-pos,#2E9E5B)' : 'var(--cm-neg,#C0492F)'}>{cmRoas(totRoas)}</CmNum></td>
                    <td></td><td></td>
                  </tr>
                </tfoot>
              )}
            ...
            {CM_PLATAFORMAS.map((p) => {
              ...
              return (
                <button key={p} onClick={() => setPlataforma(p)} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '0 14px', height: 44, borderRadius: 'var(--r-sm)', cursor: 'pointer', border: `1.5px solid ${on ? c : 'var(--border-strong)'}`, background: on ? c + '14' : 'var(--surface)' }}>
                  ...
                  <span style={{ fontSize: 13.5, fontWeight: 600, color: on ? 'var(--text)' : 'var(--text-muted)' }}>{p}</span>
                </button>
              );
            })}
          </div>
    ...
    <CmModal width={540} title="Registrar gasto" sub="Lança o investimento da campanha como despesa." onClose={onClose}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 15 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 11, padding: '12px 14px', background: 'var(--accent-soft)', border: '1px solid rgba(192,161,71,0.3)', borderRadius: 'var(--r-sm)' }}>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.5 }}>Este lançamento cria uma <strong style={{ color: 'var(--text)', fontWeight: 600 }}>despesa em Financeiro</strong> (categoria Marketing) na conta selecionada — sem dupla digitação.</div>
        </div>
      </div>
    </CmModal>
  );
