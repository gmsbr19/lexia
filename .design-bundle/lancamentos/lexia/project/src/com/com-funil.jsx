          ...
          <div key={s.key}>
            <button onClick={() => onStage(s.key)} className="cm-funnel-row" style={{
              ...
              <div style={{ width: 96, flexShrink: 0 }}>
                ...
                <div style={{ fontSize: 10.5, color: 'var(--text-subtle)', marginTop: 2, paddingLeft: 15 }}>{cmPct(s.conv, 0)} da etapa</div>
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ height: 36, borderRadius: 8, position: 'relative', background: 'var(--bg-sunken)', overflow: 'hidden' }}>
                  <div style={{ width: `${w}%`, height: '100%', background: `linear-gradient(90deg, ${s.color}, ${s.color}cc)`, borderRadius: 8, display: 'flex', alignItems: 'center', paddingLeft: 12, minWidth: 44 }}>
                    <span style={{ fontSize: 14, fontWeight: 700, color: '#fff', fontFamily: 'var(--font-mono)', fontFeatureSettings: '"tnum"' }}>{s.count}</span>
                  </div>
                </div>
              </div>
              <div style={{ width: 110, flexShrink: 0, textAlign: 'right' }}>
                <CmNum size={13} weight={600}>{cmCompact(s.value)}</CmNum>
                <div style={{ fontSize: 10.5, color: 'var(--text-subtle)', marginTop: 1 }}>{i === 4 ? 'contratado' : 'estimado'}</div>
              </div>
      ...
      <div style={{ flex: 1, minHeight: 0, overflow: 'auto' }}><CmFrame>
        <div className="card" style={{ padding: 20 }}>
          <CmEmpty icon="funnel" title="Funil vazio neste período" desc="Nenhum lead entrou no período selecionado. Cadastre leads ou ajuste o período para visualizar o funil de vendas."
            action={<button className="btn btn-primary" onClick={onLead} style={{ height: 34, fontSize: 12.5, marginTop: 4 }}><Icon name="userPlus" size={14} />Novo lead</button>} />
        </div>
      </CmFrame></div>
    );
  }
    ...
    <div style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
      <CmFrame>
        <div style={{ marginBottom: 20 }}>
          <h1 style={{ margin: 0, fontSize: 23, fontWeight: 600, letterSpacing: '-0.025em', color: 'var(--text)' }}>Funil de vendas · {scopeLabel.title}</h1>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div className="card" style={{ padding: '18px 22px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8, marginBottom: 12 }}>
                <span style={{ fontSize: 27, fontWeight: 700, color: 'var(--cm-pos,#2E9E5B)', fontFamily: 'var(--font-mono)', letterSpacing: '-0.025em' }}>{cmPct(winRate, 0)}</span>
                <span style={{ fontSize: 12.5, color: 'var(--text-muted)' }}>taxa de ganho · {f.ganho} ganhos / {f.perdido} perdas</span>
              </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                <button className="cm-rowlink" onClick={() => onStage('ganho')} style={{ textAlign: 'left', border: '1px solid var(--border)', background: 'var(--bg-soft)', borderRadius: 10, padding: '11px 13px', cursor: 'pointer' }}>
                  <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ width: 7, height: 7, borderRadius: '50%', background: '#2E9E5B' }} />Ganhos</div>
                  <div style={{ fontSize: 19, fontWeight: 700, color: 'var(--text)', marginTop: 4 }}>{f.ganho}</div>
                  <CmNum size={12} weight={600} color="var(--cm-pos,#2E9E5B)">{cmCompact(f.ganhoValor)}</CmNum>
                </button>
                <button className="cm-rowlink" onClick={() => onStage('perdido')} style={{ textAlign: 'left', border: '1px solid var(--border)', background: 'var(--bg-soft)', borderRadius: 10, padding: '11px 13px', cursor: 'pointer' }}>
                  <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}><span style={{ width: 7, height: 7, borderRadius: '50%', background: '#C0492F' }} />Perdas</div>
                  <div style={{ fontSize: 19, fontWeight: 700, color: 'var(--text)', marginTop: 4 }}>{f.perdido}</div>
                  <CmNum size={12} weight={600} color="var(--text-muted)">{cmCompact(f.perdidoValor)} perdidos</CmNum>
                </button>
              </div>
            </div>
            ...
            <div className="card" style={{ padding: '18px 22px' }}>
              ...
              {f.gargalos.length === 0 ? <div style={{ padding: '12px 0', textAlign: 'center', fontSize: 12.5, color: 'var(--text-subtle)' }}>Sem gargalos relevantes. 🎉</div> : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {f.gargalos.slice(0, 3).map((g, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
                      <span style={{ width: 22, height: 22, borderRadius: 6, background: i === 0 ? 'rgba(192,73,47,0.12)' : 'var(--bg-sunken)', color: i === 0 ? '#C0492F' : 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 700, flexShrink: 0 }}>{i + 1}</span>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 12.5, fontWeight: 500, color: 'var(--text)' }}>{g.from} → {g.to}</div>
                      ...
                      <div style={{ textAlign: 'right', flexShrink: 0 }}>
                        ...
                        <div style={{ fontSize: 10.5, color: 'var(--text-subtle)' }}>−{g.drop} leads</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="card" style={{ padding: '18px 22px' }}>
              ...
              {f.motivos.length === 0 ? <div style={{ padding: '12px 0', textAlign: 'center', fontSize: 12.5, color: 'var(--text-subtle)' }}>Nenhuma perda registrada.</div> : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {f.motivos.map((m) => (
                    <div key={m.motivo} style={{ display: 'flex', alignItems: 'center', gap: 11 }}>
                      ...
                      <span style={{ fontSize: 11.5, fontWeight: 600, color: 'var(--text-muted)', width: 18, textAlign: 'right', flexShrink: 0 }}>{m.count}</span>
                    </div>
                  ))}
                </div>
              )}
