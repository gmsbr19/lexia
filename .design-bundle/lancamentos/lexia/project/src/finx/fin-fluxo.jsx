    ...
    <div style={{ display: 'flex', alignItems: 'flex-end', gap: 0, height: 172, padding: '0 2px' }}>
      {data.map((d, i) => (
        <div key={d.key} style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6, opacity: d.proj ? 0.55 : 1 }}>
          ...
          <span style={{ fontSize: 10.5, color: 'var(--text-subtle)', whiteSpace: 'nowrap', visibility: ticks.has(i) ? 'visible' : 'hidden' }}>{fxTickLabel(d, i)}{d.proj ? '*' : ''}</span>
        </div>
      ))}
    </div>
  );
...
const FxLegend = ({ items }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
    {items.map((it) => (
      <span key={it.label} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 11.5, color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>
    ...
    <div style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
      <FxFrame>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16, marginBottom: 20, flexWrap: 'wrap' }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 21, fontWeight: 600, letterSpacing: '-0.025em', color: 'var(--text)' }}>Fluxo de caixa</h1>
            <p style={{ margin: '4px 0 0', fontSize: 13, color: 'var(--text-muted)' }}>{span} · meses com * são projeção, com recorrentes em aberto</p>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 11.5, color: 'var(--text-subtle)', fontWeight: 500 }}>Horizonte</span>
                ...
                <tbody>
                  {data.map((d) => (
                    <tr key={d.key} style={{ opacity: d.proj ? 0.66 : 1 }}>
                      <td style={{ padding: '9px 14px', fontSize: 12.5, fontWeight: 500, color: 'var(--text)', textTransform: 'capitalize', borderTop: '1px solid var(--border)' }}>{d.label}/{d.ano.slice(2)}{d.proj && <span style={{ marginLeft: 6, fontSize: 10, color: 'var(--accent)' }}>proj.</span>}</td>
                      ...
                      <td style={{ padding: '9px 14px', textAlign: 'right', borderTop: '1px solid var(--border)' }}><span style={{ fontSize: 12, ...FX_NUM, fontWeight: 600, color: d.saldo >= 0 ? 'var(--text)' : 'var(--fin-neg,#C0492F)' }}>{fxCompact(d.saldo)}</span></td>
                      <td style={{ padding: '9px 14px', textAlign: 'right', borderTop: '1px solid var(--border)' }}><span style={{ fontSize: 12, ...FX_NUM, fontWeight: 600, color: d.acc >= 0 ? 'var(--accent)' : 'var(--fin-neg,#C0492F)' }}>{fxCompact(d.acc)}</span></td>
                    </tr>
                  ))}
                </tbody>
                <tfoot>
                  <tr>
                    <td style={{ padding: '11px 14px', fontSize: 12, fontWeight: 600, color: 'var(--text)', position: 'sticky', bottom: 0, background: 'var(--bg-soft)', borderTop: '2px solid var(--border-strong)' }}>Total</td>
                    <td style={{ padding: '11px 14px', textAlign: 'right', position: 'sticky', bottom: 0, background: 'var(--bg-soft)', borderTop: '2px solid var(--border-strong)' }}><span style={{ fontSize: 12.5, ...FX_NUM, fontWeight: 700, color: 'var(--fin-pos,#2E9E5B)' }}>{fxCompact(totEnt)}</span></td>
                    <td style={{ padding: '11px 14px', textAlign: 'right', position: 'sticky', bottom: 0, background: 'var(--bg-soft)', borderTop: '2px solid var(--border-strong)' }}><span style={{ fontSize: 12.5, ...FX_NUM, fontWeight: 700, color: 'var(--fin-neg,#C0492F)' }}>{fxCompact(totSai)}</span></td>
                    <td style={{ padding: '11px 14px', textAlign: 'right', position: 'sticky', bottom: 0, background: 'var(--bg-soft)', borderTop: '2px solid var(--border-strong)' }}><span style={{ fontSize: 12.5, ...FX_NUM, fontWeight: 700, color: 'var(--text)' }}>{fxCompact(totEnt - totSai)}</span></td>
                    <td style={{ padding: '11px 14px', textAlign: 'right', position: 'sticky', bottom: 0, background: 'var(--bg-soft)', borderTop: '2px solid var(--border-strong)' }}><span style={{ fontSize: 12.5, ...FX_NUM, fontWeight: 700, color: 'var(--accent)' }}>{fxCompact(finalAcc)}</span></td>
                  </tr>
                </tfoot>
              </table>
            </div>
