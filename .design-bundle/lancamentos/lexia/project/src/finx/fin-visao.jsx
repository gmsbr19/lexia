      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
        {groups.map((g) => (
          <div key={g.s} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            ...
            <span style={{ fontSize: 12.5, color: 'var(--text)', flex: 1 }}>{meta[g.s].l}</span>
            <span style={{ fontSize: 11, color: 'var(--text-subtle)' }}>{g.count}</span>
            <span style={{ fontSize: 12.5, fontWeight: 600, color: 'var(--text)', ...FX_NUM, width: 96, textAlign: 'right' }}>{fxCompact(g.value)}</span>
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 0', borderTop: '1px solid var(--border)' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 12.5, fontWeight: 500, color: 'var(--text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{it.desc}</div>
...
const FxInadimplencia = ({ allItems, onDrill }) => {
  ...
  return (
    <div className="card" style={{ padding: '18px 22px' }}>
      ...
      {over.length === 0 ? (
        <div style={{ padding: '18px 0', textAlign: 'center', fontSize: 12.5, color: 'var(--text-subtle)' }}>Sem inadimplência. 🎉</div>
      ) : (
        <React.Fragment>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 16 }}>
            <span style={{ fontSize: 26, fontWeight: 700, color: 'var(--fin-neg,#C0492F)', letterSpacing: '-0.025em', ...FX_NUM }}>{fxCompact(total)}</span>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 9, marginBottom: 16 }}>
            {buckets.map((b) => (
              <button key={b.label} onClick={() => b.count > 0 && onDrill({ dir: 'in', stat: 'vencido', aging: b.label })} className={b.count > 0 ? 'fx-deb' : ''}
                ...
                <span style={{ fontSize: 11.5, color: 'var(--text-muted)', width: 64, flexShrink: 0 }}>{b.label}</span>
                ...
                <span style={{ fontSize: 10.5, color: 'var(--text-subtle)', width: 16, textAlign: 'right' }}>{b.count}</span>
                <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)', ...FX_NUM, width: 74, textAlign: 'right' }}>{fxCompact(b.value)}</span>
              </button>
            ))}
          </div>
          <div style={{ borderTop: '1px solid var(--border)', paddingTop: 12 }}>
            <div style={{ fontSize: 10.5, color: 'var(--text-subtle)', textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 600, marginBottom: 4 }}>Maiores devedores · clique para cobrar</div>
            {top.map((t) => (
              <button key={t.party} onClick={() => onDrill({ dir: 'in', stat: 'vencido', q: t.party })} className="fx-deb"
                ...
                <span style={{ width: 24, height: 24, borderRadius: '50%', background: 'rgba(192,73,47,0.12)', color: '#C0492F', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, flexShrink: 0 }}>{t.party.split(' ').map((w) => w[0]).join('').slice(0, 2).toUpperCase()}</span>
                <span style={{ fontSize: 12.5, color: 'var(--text)', flex: 1, minWidth: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{t.party}</span>
                <span style={{ fontSize: 11, color: '#C0492F', fontWeight: 600 }}>{t.dias}d</span>
                <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)', ...FX_NUM, width: 76, textAlign: 'right' }}>{fxCompact(t.value)}</span>
    ...
    <div style={{ flex: 1, minHeight: 0, overflow: 'auto' }}>
      <FxFrame>
        <div style={{ marginBottom: 20 }}>
          <h1 style={{ margin: 0, fontSize: 21, fontWeight: 600, letterSpacing: '-0.025em', color: 'var(--text)' }}>Visão geral · {scopeLabel.title}</h1>
        ...
        {vencidos.length > 0 && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 13, padding: '12px 16px', background: 'rgba(192,73,47,0.07)', border: '1px solid rgba(192,73,47,0.22)', borderRadius: 'var(--r-md)', marginBottom: 22 }}>
            ...
            <div style={{ flex: 1 }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>{vencidos.length} {vencidos.length === 1 ? 'lançamento vencido' : 'lançamentos vencidos'} · <span style={FX_NUM}>{fxMoney(vencidos.reduce((a, b) => a + b.valor, 0))}</span></span>
              <span style={{ fontSize: 12.5, color: 'var(--text-muted)' }}> — requerem cobrança ou pagamento.</span>
            </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.3fr', gap: 20, alignItems: 'start' }}>
          ...
          <div className="card" style={{ padding: '18px 22px' }}>
            <FxCardTitle title="Próximos vencimentos" sub="Em aberto, ordenados por data" right={venc.length > 0 && <span style={{ fontSize: 11.5, color: 'var(--text-subtle)' }}>{venc.length} de {items.filter((i) => !i.pago).length}</span>} />
            {venc.length === 0
              ? <div style={{ padding: '24px 0', textAlign: 'center', fontSize: 12.5, color: 'var(--text-subtle)' }}>Nada em aberto neste período. 🎉</div>
