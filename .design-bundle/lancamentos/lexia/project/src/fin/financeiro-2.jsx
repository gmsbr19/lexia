...
const CostBar = ({ c, max }) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
    ...
    <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text)', fontFamily: 'var(--font-mono)', fontFeatureSettings: '"tnum"', width: 78, textAlign: 'right' }}>{brlCompact(c.value)}</span>
  </div>
);
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, alignItems: 'start' }}>
        <div className="card" style={{ overflow: 'hidden', padding: 0 }}>
          <div style={{ padding: '14px 16px', borderBottom: '1px solid var(--border)', background: 'var(--bg-soft)' }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text)' }}>Resultado do exercício</div>
          </div>
...
const SemFeeRow = ({ r }) => (
  <tr style={{ borderTop: '1px solid var(--border)' }}>
    ...
    <Td align="right">
      <button className="btn btn-secondary" style={{ height: 28, fontSize: 11.5, padding: '0 10px', opacity: 0.5, cursor: 'not-allowed' }} disabled>
  ...
  <PageFrame>
    <PageHeader title="Casos sem fee"
      ...
      right={<div className="card" style={{ padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 10 }}>
        ...
        <div><div style={{ fontSize: 19, fontWeight: 600, color: 'var(--text)', letterSpacing: '-0.02em', lineHeight: 1 }}>5</div><div style={{ fontSize: 10.5, color: 'var(--text-subtle)' }}>casos sem fee</div></div>
      </div>} />
    <TableShell
...
const ImportSummaryCard = ({ icon, label, value, sub, tone }) => (
  <div className="card" style={{ padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 8 }}>
    ...
    <div style={{ fontSize: 23, fontWeight: 600, color: 'var(--text)', letterSpacing: '-0.025em', fontFeatureSettings: '"tnum"' }}>{value}</div>
...
const FlaggedRow = ({ r }) => (
  <tr style={{ borderTop: '1px solid var(--border)', opacity: 0.78 }}>
    ...
    <Td muted>{r.conta}</Td>
    <Td><span style={{ fontSize: 11.5, color: '#C77E1F' }}>{r.motivo}</span></Td>
...
const FinImportacao = () => finShell('import',
  <PageFrame>
    <PageHeader title="Importação"
      ...
      right={<button className="btn btn-primary" style={{ height: 34, fontSize: 12.5 }}><Icon name="refreshCw" size={13} />Reimportar backup</button>} />
    ...
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
      <div>
        <div style={{ fontSize: 13.5, fontWeight: 600, color: 'var(--text)', letterSpacing: '-0.01em' }}>Lançamentos sinalizados</div>
        <div style={{ fontSize: 11.5, color: 'var(--text-subtle)', marginTop: 2 }}>Excluídos dos cálculos até revisão manual</div>
      </div>
    </div>
    <TableShell
